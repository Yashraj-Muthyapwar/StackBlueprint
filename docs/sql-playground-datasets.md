...
Then open the SQL Playground and, for each engine, select Ergast, Marvel, and Olist. Confirm that the expected table count appears in Schema Explorer, run a small `SELECT ... LIMIT 10`, and test one join. For Ergast, the 2024 calendar query should return 24 races. For Marvel, Schema Explorer should show five tables, including 205,607 `character_links`. For Olist, the `orders` table should contain 99,441 rows.
