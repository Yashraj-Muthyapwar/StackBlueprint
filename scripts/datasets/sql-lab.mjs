...
const column = (name, type, notNull = false, defaultValue) => ({
  name,
  type,
  notNull,
  defaultValue,
...
export async function buildSqlLab() {
  ...
  for (let id = 1; id <= COUNTS.customers; id++) {
    ...
    customers.push([
      ...
      first,
      last,
      `${first.toLowerCase()}.${last.toLowerCase()}${id}@shopflow.test`,
      pick(COUNTRIES, rng),
      timestamp(Math.floor(rng() * 365), Math.floor(rng() * 24), Math.floor(rng() * 60)),
    ]);
  }
  ...
  for (let id = 1; id <= COUNTS.addresses; id++) {
    ...
    addresses.push([
      id,
      customerId,
      id <= COUNTS.customers ? "shipping" : "billing",
      `${100 + id} Market Street`,
      pick(CITIES, rng),
      "Central",
      `${10000 + (id % 89999)}`,
      customers[customerId - 1][4],
      id <= COUNTS.customers ? "true" : "false",
    ]);
  }
  ...
  for (let id = 1; id <= COUNTS.products; id++) {
    ...
    products.push([
      id,
      categoryId,
      `${CATEGORIES[categoryId - 1][2]} Product ${id}`,
      price,
      cost,
      id % 19 === 0 ? "false" : "true",
    ]);
  ...
  for (let id = 8; id <= COUNTS.employees; id++) {
    ...
    employees.push([
      id,
      managerId,
      `${FIRST_NAMES[id % FIRST_NAMES.length]} ${LAST_NAMES[Math.floor(id / 3) % LAST_NAMES.length]}`,
      department,
      id <= 20 ? "Team Lead" : "Specialist",
    ]);
  }
  ...
  for (let orderId = 1; orderId <= COUNTS.orders; orderId++) {
        ...
        : roll < 0.15
          ...
          : roll < 0.3
            ...
            : roll < 0.5
              ...
              : "delivered";
    const customerId =
      orderId % 53 === 0 ? 1 + Math.floor(rng() * 40) : 1 + Math.floor(rng() * COUNTS.customers);
    ...
    const paymentStatus =
      status === "pending" ? "pending" : status === "cancelled" ? "failed" : "successful";
    payments.push([
      orderId,
      orderId,
      paymentStatus === "pending" ? "" : orderDate,
      paymentStatus === "successful" ? totalAmount : "0.00",
      paymentStatus,
    ]);
    ...
    if (status !== "cancelled") {
      ...
      const shippedAt =
        shipmentStatus === "pending" ? "" : timestampAfter(orderDate, 6 + Math.floor(rng() * 72));
      const deliveredAt =
        shipmentStatus === "delivered"
          ? timestampAfter(shippedAt, 1 + Math.floor(rng() * 120))
          : "";
      shipments.push([
        shipments.length + 1,
        orderId,
        shippedAt,
        deliveredAt,
        shipmentStatus === "pending" ? "" : pick(["NorthStar", "ParcelCo", "SwiftShip"], rng),
        shipmentStatus,
      ]);
    }
  }
    ...
    for (let warehouseId = 1; warehouseId <= COUNTS.warehouses; warehouseId++) {
      ...
      const quantity =
        productId % 37 === 0
          ...
          : productId % 11 === 0
            ? reorderLevel - 2
            : reorderLevel + 12 + Math.floor(rng() * 140);
  ...
  const definitions = [
    table(
      ...
      [
        ...
        column("first_name", "VARCHAR(50)", true),
        column("last_name", "VARCHAR(50)", true),
        column("email", "VARCHAR(150)", true),
        column("country", "VARCHAR(50)", true),
        column("created_at", "TIMESTAMP", true),
      ],
      ["customer_id"],
      ['PRIMARY KEY ("customer_id")', 'UNIQUE ("email")'],
    ),
    table(
      "categories",
      ...
      [
        ...
        column("category_name", "VARCHAR(100)", true),
      ],
      ["category_id"],
      [
    ...
    table(
      "products",
      ["product_id", "category_id", "product_name", "price", "cost", "active"],
      products,
      [
        column("product_id", "INTEGER", true),
        column("category_id", "INTEGER", true),
        column("product_name", "VARCHAR(150)", true),
        column("price", "DECIMAL(10,2)", true),
        column("cost", "DECIMAL(10,2)", true),
        column("active", "BOOLEAN", true, "TRUE"),
      ],
      ["product_id"],
      [
    ...
    table(
      "employees",
      ["employee_id", "manager_id", "employee_name", "department", "title"],
      employees,
      [
        column("employee_id", "INTEGER", true),
        column("manager_id", "INTEGER"),
        column("employee_name", "VARCHAR(100)", true),
        column("department", "VARCHAR(50)", true),
        column("title", "VARCHAR(100)", true),
      ],
      ["employee_id"],
      [
    ...
    table(
      ...
      [
        ...
        column("customer_id", "INTEGER", true),
        column("address_type", "VARCHAR(20)", true),
        column("line1", "VARCHAR(150)", true),
        column("city", "VARCHAR(80)", true),
        column("region", "VARCHAR(80)", true),
        column("postal_code", "VARCHAR(20)", true),
        column("country", "VARCHAR(50)", true),
        column("is_default", "BOOLEAN", true, "FALSE"),
      ],
      ["address_id"],
      [
    ...
    table(
      ...
      [
        ...
        column("customer_id", "INTEGER", true),
        column("order_date", "TIMESTAMP", true),
        column("status", "VARCHAR(20)", true),
        column("total_amount", "DECIMAL(12,2)", true),
      ],
      ["order_id"],
      [
    ...
    table(
      ...
      [
        ...
        column("unit_price", "DECIMAL(10,2)", true),
      ],
      ["order_id", "product_id"],
      [
    ...
    table(
      ...
      [
        ...
        column("amount", "DECIMAL(12,2)", true),
        column("status", "VARCHAR(20)", true),
      ],
      ["payment_id"],
      [
    ...
    table(
      "shipments",
      ["shipment_id", "order_id", "shipped_at", "delivered_at", "carrier", "status"],
      shipments,
      [
        column("shipment_id", "INTEGER", true),
        column("order_id", "INTEGER", true),
        column("shipped_at", "TIMESTAMP"),
        column("delivered_at", "TIMESTAMP"),
        column("carrier", "VARCHAR(50)"),
        column("status", "VARCHAR(20)", true),
      ],
      ["shipment_id"],
      [
        ...
        'CHECK (("status" = \'pending\' AND "shipped_at" IS NULL AND "delivered_at" IS NULL) OR ("status" = \'shipped\' AND "shipped_at" IS NOT NULL AND "delivered_at" IS NULL) OR ("status" = \'delivered\' AND "shipped_at" IS NOT NULL AND "delivered_at" IS NOT NULL AND "delivered_at" >= "shipped_at"))',
      ],
    ),
    table(
      "inventory",
      ...
      [
        column("product_id", "INTEGER", true),
        column("warehouse_id", "INTEGER", true),
        column("quantity", "INTEGER", true),
        column("reorder_level", "INTEGER", true),
      ],
  ...
  console.log(
    `  ${tables.length} tables · ${tables.reduce((sum, value) => sum + value.rows, 0).toLocaleString()} rows · ${mb(bytes)} gzipped (${orderItemCount.toLocaleString()} order items)`,
  );
