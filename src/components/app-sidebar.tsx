                    ...
                    <div className="flex flex-1 items-center gap-2">
                      {typeof cat.icon === "string" ? (
                        <img
                          src={cat.icon}
                          alt={`${cat.title} logo`}
                          className="size-3 object-contain drop-shadow-sm opacity-80"
                        />
                      ) : (
    ...
    <Collapsible open={open} onOpenChange={setOpen} asChild>
      <SidebarMenuItem>
        <CollapsibleTrigger asChild>
          <SidebarMenuButton
            tooltip={title}
            className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted-foreground/80 hover:text-foreground"
          >
            <ChevronRight
...
function NestedPatternItem({
  ...
  if (!hasLessons) {
    ...
    return (
      <SidebarMenuSubItem>
        <SidebarMenuSubButton asChild isActive={patternActive}>
          <Link to="/patterns/$pattern" params={{ pattern: pat.slug }} onClick={onNavigate}>
            <ChevronRight className="size-3.5" />
            <span>{pat.title}</span>
          </Link>
        </SidebarMenuSubButton>
              ...
              <SidebarMenuSubItem key={pat.slug}>
                <SidebarMenuSubButton asChild className="text-muted-foreground/70">
                  {isSql ? (
                    ...
                    <Link to="/tracks/$track" params={{ track: cat.slug }} onClick={onNavigate}>
                      <Lock className="size-3" />
                      <span>{pat.title}</span>
                    </Link>
                  )}
