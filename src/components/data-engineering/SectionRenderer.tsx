...
function InteractivePipelineFlow({ steps }: { steps: { title: string; description: string }[] }) {
  ...
  return (
    <div className="my-6 rounded-xl border border-hairline bg-slate-50/50 dark:bg-surface shadow-sm overflow-hidden">
      ...
      <div className="lesson-supporting p-5 text-foreground/90">
...
export function SectionRenderer({ section, onQuizActiveChange }: { section: Section; onQuizActiveChange?: (active: boolean) => void }) {
  switch (section.kind) {
      ...
      return (
        <section className="space-y-3">
          ...
          {section.body.map((p, i) => {
            if (p.startsWith('### ')) {
              return (
                <h3 key={i} className="lesson-subsection-title mt-8 mb-2 font-semibold tracking-tight text-foreground">
          ...
          {section.items.map((item, i) => {
            ...
            return (
              <div key={i} className={`rounded-xl border ${colors[item.provider]} p-4 flex flex-col shadow-sm`}>
                ...
                <div className="lesson-supporting text-foreground/90">
    ...
    case "pipeline-flow":
      return <InteractivePipelineFlow steps={section.steps} />;
