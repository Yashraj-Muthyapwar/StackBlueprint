...
const CONTAINER_ID = "8b3c2c5d8e1a4f9e2b7c8d6a1e5f3b9c7a2d1e0f6b5c4a3d2e1f0a9b8c7d6e5f4";
...
function buildLines(): Line[] {
  ...
  curl.forEach(([text, color], i) => L.push(out(T.curlOut + i * 110, text, color)));
  ...
  const logs: Array<[string, string?]> = [
    ...
    ["/docker-entrypoint.sh: Launching /docker-entrypoint.d/20-envsubst-on-templates.sh", C.dim],
    ["/docker-entrypoint.sh: Launching /docker-entrypoint.d/30-tune-worker-processes.sh", C.dim],
  ...
  logs.forEach(([text, color], i) => L.push(out(T.logsOut + i * 95, text, color)));
...
const STAGES: Stage[] = [
  {
    ...
    body: (
      <>
        ...
        <br />
        The CLI sends a request to the Docker Daemon via the Docker API (Unix socket).
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Receives the request, validates it, sets up networking (the {mono("-p 8080:80", C.green)}{" "}
        port mapping), and forwards it to containerd via gRPC.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Checks for the image locally; pulls it from the Registry if missing. Its snapshotter then
        stacks the image layers with overlay2 and adds the container's writable layer on top.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        A dedicated shim process is created for this container. It will manage the container's
        stdin/stdout, signals and lifecycle.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        The shim asks runc to create the container. runc sets up namespaces, cgroups and mounts via
        the Linux Kernel, starts the container process — then exits. The shim stays behind as the
        process's parent.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Applies isolation (namespaces, cgroups, capabilities, seccomp, AppArmor/SELinux) and starts
...
function ProgressLine({ t, p }: { t: number; p: NonNullable<Line["progress"]> }) {
  ...
  const bar =
    ...
    (filled > 0 && filled < width ? ">" : filled >= width ? "=" : "");
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div
          style={{ fontSize: 11.5, lineHeight: 1.5, color: lit ? C.dim : C.faint, marginTop: 2 }}
        >
          {stage.body}
        </div>
      </div>
                  ...
                  {i === 2 ? (
                    <div className="sbduh-registry-col">
                      ...
                      >
                        <div
                          style={{
                            color: pulling ? C.purple : C.faint,
                            marginBottom: 6,
                            transition: "color .4s",
                          }}
                        >
                        ...
                        <div
                          style={{ fontSize: 10.5, lineHeight: 1.45, color: C.faint, marginTop: 6 }}
                        >
                      ...
                      >
                        ...
                        <line
                          ...
                          style={
                            pulling ? { animation: "sbduh-dash 0.7s linear infinite" } : undefined
                          }
                        />
                        <line
                          x1="27"
            ...
            <div className="sbduh-stage-main">
              ...
              <div
                style={{
                  ...
                  animation:
                    started && t < T.done ? "sbduh-startglow 2s ease-in-out infinite" : "none",
                }}
              >
                <div
                  style={{
                ...
                <div>
                  <div
                    style={{ fontWeight: 700, fontSize: 13.5, color: started ? C.green : C.dim }}
                  >
                    Container Started ✓
                  </div>
                  <div
                    style={{
                      fontSize: 11.5,
                      lineHeight: 1.5,
                      color: started ? C.dim : C.faint,
                      marginTop: 2,
                    }}
                  >
                    Status flows back: shim → containerd → dockerd → CLI. The container is now
                    running.
                  </div>
                </div>
              </div>
