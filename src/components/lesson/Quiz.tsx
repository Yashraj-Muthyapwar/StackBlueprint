...
function parsePythonError(errorText: string): { coreIssue: string; hint: string } | null {
  ...
  if (
    ...
    (errorText.includes("takes no arguments") || errorText.match(/takes \d+ positional arguments/))
  ) {
    return {
      coreIssue:
        "Your class was initialized incorrectly because Python couldn't find a valid __init__ method with the right parameters.",
  ...
  if (
    ...
    errorText.includes("required positional argument")
  ) {
    return {
...
function stableOptionShuffle(question: QuizQuestion) {
  ...
  let seed = Array.from(question.id).reduce(
...
function NormalQuiz({
  ...
  const handleCommandSubmit = async () => {
    if (
      (!currentQuestion.commandAnswer && !currentQuestion.interactiveCode) ||
      isAnswered ||
      isEvaluating
    )
      return;
  ...
  if (isFinished) {
    ...
    if (percentage === 100) feedback = "Perfect! You nailed every concept.";
      ...
      feedback = "Good effort! A quick review might help you master the rest.";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mb-6 flex items-center justify-between">
        <div
          className={cn(
            ...
            data.isFinalQuiz ? "text-sm font-medium tracking-[0.2em]" : "text-xs",
          )}
        >
      ...
      <div
        className={cn(
          "w-full rounded-full",
          data.isFinalQuiz ? "mb-10 h-1.5 bg-surface-3/50" : "mb-6 h-1 bg-surface-2",
        )}
      >
        <motion.div
      ...
      <h3
        className={cn(
          "relative z-10 leading-relaxed text-foreground",
          data.isFinalQuiz
      ...
      {currentQuestion.interactiveCode ? (
        ...
        <div className="flex flex-col gap-4">
          <div className="relative">
            {(() => {
              const ansStr = Array.isArray(currentQuestion.commandAnswer)
                ? currentQuestion.commandAnswer[0]
                : currentQuestion.commandAnswer;
        ...
        <div className="space-y-3">
          {shuffledOptions.map((opt, i) => {
            ...
            if (data.isFinalQuiz && !isAnswered) {
              btnClass =
                "border-hairline/40 bg-surface/30 hover:border-mint/30 hover:bg-surface hover:shadow-md";
            }

            if (isAnswered) {
              if (isCorrect) {
                btnClass = data.isFinalQuiz
                  ? "border-mint/50 bg-mint/10 text-mint shadow-sm"
            ...
            return (
              <button
                ...
                disabled={isAnswered}
                className={cn(
                  "relative z-10 flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition-all duration-300",
                  btnClass,
                  data.isFinalQuiz ? "md:px-6 md:py-5" : "text-sm",
                )}
              >
                <div className="flex items-center gap-4">
                  ...
                  <span
                    className={cn(
                      isAnswered && isCorrect ? "font-medium" : "",
                ...
                {isAnswered && isCorrect && (
                  <CheckCircle2
                    className={cn("shrink-0 text-mint", data.isFinalQuiz ? "size-6" : "size-4")}
                  />
                )}
                {isAnswered && isSelected && !isCorrect && (
                  <XCircle
                    className={cn("shrink-0 text-rose-500", data.isFinalQuiz ? "size-6" : "size-4")}
                  />
                )}
              </button>
            );
            ...
            >
              ...
              {currentQuestion.commandAnswer && !isCommandCorrect && (
                <p className="mb-2 text-sm font-mono text-rose-500">
                  Expected:{" "}
                  {Array.isArray(currentQuestion.commandAnswer)
                    ? currentQuestion.commandAnswer[0]
                    : currentQuestion.commandAnswer}
                </p>
              )}
                        ...
                        {friendlyError ? (
                          ...
                          <>
                            <p className="mb-2 text-sm font-mono text-rose-500 whitespace-pre-wrap">
                              ...
                              <br />
                              {currentQuestion.expectedOutput || "None"}
                            </p>
                            <p className="mb-2 text-sm font-mono text-rose-500 whitespace-pre-wrap">
                              <span className="font-bold">Your code produced:</span>
                              <br />
    ...
    for (let idx = 0; idx < shuffledQuestions.length; idx++) {
      ...
      if (q.interactiveCode) {
        try {
          ...
          const fullCode =
            (commandAnswers[idx] || q.initialCode || "") + "\n\n" + (q.testCode || "");
          await pyodide.runPythonAsync(fullCode);
    ...
    <div className="mt-8 w-full max-w-3xl mx-auto">
      {isSubmitted && (
        <div className="mb-16 rounded-xl border border-hairline bg-surface p-8 text-center">
          <div className="text-5xl font-bold tracking-tight text-mint mb-2">
            {score}{" "}
            <span className="text-3xl text-muted-foreground">/ {data.questions.length}</span>
          </div>
              ...
              <div className="flex-1">
                ...
                {q.interactiveCode ? (
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="relative">
                      {(() => {
                        const ansStr = Array.isArray(q.commandAnswer)
                          ? q.commandAnswer[0]
                          : q.commandAnswer;
                        const isMultiline = ansStr && ansStr.includes("\n");
                  ...
                  <div className="space-y-3 mt-4">
                    {q.shuffledOptions.map((opt: any, oIndex: number) => {
                      ...
                      let rowClass =
                        "border border-hairline bg-surface hover:border-mint/40 hover:bg-surface-2/50 text-foreground/90 shadow-sm";
                      let indicatorClass = "border-hairline bg-surface-2 text-muted-foreground";

                      if (isSelected) {
                        rowClass =
                          "border-mint bg-mint/5 text-foreground shadow-md ring-1 ring-mint/50";
                      ...
                      return (
                        <button
                          ...
                          disabled={isSubmitted}
                          className={cn(
                            "group flex w-full items-start gap-5 rounded-xl px-5 py-4 text-left transition-all duration-200",
                            rowClass,
                          )}
                        >
                          <div
                            className={cn(
                              "mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border text-sm font-semibold transition-colors shadow-sm",
                              indicatorClass,
                            )}
                          >
                          ...
                          {isSubmitted && isSelected && !isCorrect && (
                            <XCircle className="ml-auto size-6 shrink-0 text-rose-500" />
                          )}
                        </button>
                      );
                    })}
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          (
                            q.interactiveCode
                              ? evaluationOutputs[qIndex] === (q.expectedOutput || "").trim()
                              : q.commandAnswer
                                ? isCommandCorrect(q, commandAnswers[qIndex] || "")
                                : selectedAnswers[qIndex] !== undefined &&
                                  q.shuffledOptions[selectedAnswers[qIndex]].isCorrect
                          )
                      ...
                      >
                        ...
                        {q.commandAnswer && !isCommandCorrect(q, commandAnswers[qIndex] || "") && (
                          <p className="mb-4 text-sm font-mono text-rose-500">
                            Expected:{" "}
                            {Array.isArray(q.commandAnswer) ? q.commandAnswer[0] : q.commandAnswer}
                          </p>
                        )}
                        {q.interactiveCode &&
                        evaluationOutputs[qIndex] !== undefined &&
                        evaluationOutputs[qIndex] !== (q.expectedOutput || "").trim()
                          ? (() => {
                                  ...
                                  {friendlyError ? (
                                    ...
                                    <>
                                      ...
                                      <p className="mb-2 text-sm font-mono text-rose-500 whitespace-pre-wrap">
                                        <span className="font-bold">Your code produced:</span>
                                        <br />
                                        {evaluationOutputs[qIndex] || "No output"}
                                      </p>
                                    </>
                                  )}
                                </div>
