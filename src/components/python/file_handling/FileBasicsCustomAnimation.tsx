  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-violet/30 bg-violet/5 font-mono text-sm shadow-sm w-full max-w-lg">
        <span className="text-mint">contacts</span> = [
        <span className="text-amber">'Ada Reed'</span>,{" "}
        <span className="text-amber">'John Doe'</span>,{" "}
        <span className="text-amber">'Bob Smith'</span>]
      </div>
      <div className="flex justify-center gap-16 w-full max-w-2xl">
        <div className="flex flex-col items-center gap-3">
          <div className="p-6 rounded-2xl bg-violet/10 border border-violet/20 flex flex-col items-center relative min-w-[160px] shadow-sm">
            ...
            >
              <div className="px-3 py-1.5 rounded bg-surface border border-hairline text-sm shadow-sm text-foreground text-center">
                Ada Reed
              </div>
              <div className="px-3 py-1.5 rounded bg-surface border border-hairline text-sm shadow-sm text-foreground text-center">
                John Doe
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Variables only live in temporary memory. When the script exits, they disappear.
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-mint/30 bg-mint/5 font-mono text-sm shadow-sm w-full max-w-lg">
        ...
        <span className="text-amber">'contacts.txt'</span>, <span className="text-amber">'w'</span>){" "}
        <span className="text-blue-400">as</span> file:
        <br />
        &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-blue-400">for</span> name{" "}
        <span className="text-blue-400">in</span> <span className="text-mint">contacts</span>:<br />
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;file.write(name +{" "}
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Opening in <code className="bg-surface-2 px-1 rounded">"w"</code> mode permanently writes
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-sky-500/30 bg-sky-500/5 font-mono text-sm shadow-sm w-full max-w-lg">
        <span className="text-blue-400">with</span> <span className="text-blue-400">open</span>(
        <span className="text-amber">'contacts.txt'</span>, <span className="text-amber">'r'</span>){" "}
        <span className="text-blue-400">as</span> file:
        <br />
        &nbsp;&nbsp;&nbsp;&nbsp;saved = file.read()
        <br />
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Using <code className="bg-surface-2 px-1 rounded">"r"</code> mode reads the stored bytes
        back into a string in memory.
      </p>
    </motion.div>
  );
}
