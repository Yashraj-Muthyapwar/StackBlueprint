  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-sky-500/30 bg-sky-500/5 font-mono text-[13px] shadow-sm w-full max-w-lg">
        file.read(6)
        <br />
        pos = file.<span className="text-sky-400 font-bold">tell()</span>
        <br />
        print(f"Cursor is at byte: {"{pos}"}"){" "}
        <span className="text-muted-foreground"># Cursor is at byte: 6</span>
      </div>
        ...
        <div className="flex gap-1 relative p-4 bg-surface rounded-xl border border-hairline shadow-sm">
          {characters.map((char, i) => (
            <div key={i} className="flex flex-col items-center gap-2">
              <div
                className={`w-8 h-10 flex items-center justify-center font-mono font-bold text-lg rounded ${i < 6 ? "bg-surface-2 text-muted-foreground/40" : "bg-surface border border-hairline text-foreground"}`}
              >
                {char === " " ? "␣" : char}
              </div>
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-sm text-center">
        <code>tell()</code> returns the <strong>current byte position</strong> of the cursor.
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-violet/30 bg-violet/5 font-mono text-[13px] shadow-sm w-full max-w-lg">
        file.read(6) <span className="text-muted-foreground"># Cursor is at 6</span>
        <br />
        file.<span className="text-violet font-bold">seek(0)</span>{" "}
        <span className="text-muted-foreground"># Jump back to 0</span>
        <br />
      </div>
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-sm text-center">
        <code>seek(0)</code> physically moves the cursor to a specific byte index, allowing you to
        re-read or overwrite data.
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-mint/30 bg-mint/5 font-mono text-[13px] shadow-sm w-full max-w-lg">
        content = file.<span className="text-mint font-bold">read(5)</span>{" "}
        <span className="text-muted-foreground"># Read 5 bytes</span>
      </div>

      <div className="flex justify-center w-full max-w-2xl relative mt-8 gap-8 items-center">
        <div className="flex flex-col items-center">
          <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2 font-bold">
            File System
          </div>
          <div className="flex gap-1 p-2 bg-surface rounded border border-hairline">
            {"HELLO WORLD".split("").map((char, i) => (
              <div
                key={i}
                className={`w-6 h-8 flex items-center justify-center font-mono font-bold text-sm rounded ${i < 5 ? "bg-mint/20 text-mint" : "bg-surface-2 text-muted-foreground/40"}`}
              >
                {char === " " ? "␣" : char}
              </div>
            ))}
        ...
        <div className="flex flex-col items-center">
          <div className="font-mono text-[10px] uppercase tracking-widest text-mint mb-2 font-bold">
            Memory
          </div>
          <motion.div
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-sm text-center">
        <code>read(n)</code> reads exactly <strong>n bytes</strong> and advances the cursor by n.
        Crucial for binary or huge files.
      </p>
    </motion.div>
  );
}
