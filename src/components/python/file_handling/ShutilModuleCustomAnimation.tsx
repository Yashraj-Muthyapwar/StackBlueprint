...
import { 
  FileText, Folder, HardDrive, ArrowRight, 
...
export function ShutilModuleCustomAnimation() {
  ...
  const getPhaseName = () => {
    switch(step) {
      case 0: return "copy2()";
      case 1: return "copytree()";
      case 2: return "move()";
      case 3: return "make_archive()";
      case 4: return "copyfileobj()";
      case 5: return "rmtree()";
      default: return "";
    }
  };
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-violet/30 bg-violet/5 font-mono text-sm shadow-sm w-full max-w-lg">
        <span className="text-violet">shutil</span>.<span className="text-sky-500">copy2</span>(source, backups)
      </div>
      <div className="flex justify-center items-center gap-16 w-full max-w-3xl">
        {/* Source Folder */}
        <motion.div layoutId="source-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm relative">
           <Folder className="size-8 text-sky-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-sky-500 font-semibold mb-4">exports/</span>
        ...
        {/* Dest Folder */}
        <motion.div layoutId="dest-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm">
           <Folder className="size-8 text-violet mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-violet font-semibold mb-4">backups/</span>
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        <code className="bg-surface-2 px-1 rounded">copy2()</code> creates an exact duplicate, preserving timestamps and file metadata where the platform allows.
      </p>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 font-mono text-sm shadow-sm w-full max-w-xl text-center">
        <span className="text-emerald-500">shutil</span>.<span className="text-sky-500">copytree</span>(src, dst, <span className="text-amber">ignore</span>=<span className="text-emerald-500">ignore_patterns</span>(<span className="text-amber">'*.tmp'</span>))
      </div>
      
      <div className="flex justify-center items-center gap-12 w-full max-w-4xl relative">
        {/* Source Folder */}
        <motion.div layoutId="source-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm">
           <Folder className="size-8 text-sky-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-sky-500 font-semibold mb-4">release_source/</span>
           
           <div className="flex flex-col gap-2 w-full">
             <div className="p-2 rounded bg-surface-2 border border-hairline flex items-center gap-2">
               <FileText className="size-4" /> <span className="text-[10px] font-mono">summary.txt</span>
             </div>
             <motion.div 
               animate={{ x: [0, 10, 0] }}
             ...
             >
               <FileType2 className="size-4 text-rose-500" /> <span className="text-[10px] font-mono text-rose-500">draft.tmp</span>
             </motion.div>
           </div>
        </motion.div>
        ...
        {/* Dest Folder */}
        <motion.div layoutId="dest-folder" className="p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 flex flex-col items-center min-w-[200px] shadow-sm">
           <Folder className="size-8 text-emerald-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-emerald-500 font-semibold mb-4">staging/</span>
           ...
           <div className="flex flex-col gap-2 w-full">
             ...
             >
               <FileText className="size-4 text-emerald-600 dark:text-emerald-400" /> <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400">summary.txt</span>
             </motion.div>
             ...
             >
               <XCircle className="size-3 text-muted-foreground" /> <span className="text-[9px] font-mono text-muted-foreground italic">Ignored</span>
             </motion.div>
           </div>
        </motion.div>
      </div>
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        <code className="bg-surface-2 px-1 rounded">copytree()</code> duplicates entire directories. Using <code className="bg-surface-2 px-1 rounded">ignore_patterns</code> filters out temporary files from the backup.
      </p>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-amber/30 bg-amber/5 font-mono text-sm shadow-sm w-full max-w-lg text-center">
        <span className="text-amber">shutil</span>.<span className="text-sky-500">move</span>(incoming, processed)
      </div>
      ...
      <div className="flex justify-center items-center gap-16 w-full max-w-3xl">
        ...
        <motion.div layoutId="source-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm relative">
           <Folder className="size-8 text-sky-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-sky-500 font-semibold mb-4">incoming/</span>
        ...
        {/* Dest Folder */}
        <motion.div layoutId="dest-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm relative">
           <Folder className="size-8 text-emerald-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-emerald-500 font-semibold mb-4">processed/</span>
           ...
           <div className="h-[60px] w-full flex items-center justify-center">
             <motion.div 
              ...
              >
                ...
                <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400">batch.csv</span>
              </motion.div>
           </div>
        </motion.div>
      </div>
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        Unlike copy, <code className="bg-surface-2 px-1 rounded">move()</code> removes the original file. If crossing filesystems, it safely copies the data and then deletes the source.
      </p>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-indigo-500/30 bg-indigo-500/5 font-mono text-sm shadow-sm w-full max-w-xl text-center">
        <span className="text-indigo-400">shutil</span>.<span className="text-sky-500">make_archive</span>(<span className="text-amber">'export'</span>, <span className="text-amber">'zip'</span>, root_dir)
      </div>
      
      <div className="flex justify-center items-center gap-12 w-full max-w-3xl relative">
        {/* Staging Folder */}
        <motion.div layoutId="source-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[200px] shadow-sm relative z-10">
           <Folder className="size-8 text-indigo-400 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-indigo-400 font-semibold mb-4">staging/</span>
           
           <div className="grid grid-cols-2 gap-2 w-full">
             <div className="p-2 rounded bg-surface-2 border border-hairline flex justify-center"><FileText className="size-5" /></div>
             <div className="p-2 rounded bg-surface-2 border border-hairline flex justify-center"><FileText className="size-5" /></div>
             <div className="p-2 rounded bg-surface-2 border border-hairline flex justify-center"><FileText className="size-5" /></div>
             <div className="p-2 rounded bg-surface-2 border border-hairline flex justify-center"><FileText className="size-5" /></div>
           </div>
        ...
        <div className="flex flex-col items-center text-muted-foreground z-0">
          ...
          <span className="text-[10px] font-mono mt-2 text-indigo-400 font-bold uppercase tracking-wider">Compress</span>
        </div>

        {/* Zip File */}
        <motion.div layoutId="dest-folder" className="p-6 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 flex flex-col items-center min-w-[200px] shadow-[0_0_20px_rgba(99,102,241,0.2)]">
           <motion.div
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        <code className="bg-surface-2 px-1 rounded">make_archive()</code> compresses an entire directory tree into a single shareable file like a ZIP or TAR.
      </p>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-sky-500/30 bg-sky-500/5 font-mono text-sm shadow-sm w-full max-w-lg text-center">
        <span className="text-sky-500">shutil</span>.<span className="text-violet">copyfileobj</span>(stream, file)
      </div>
      
      <div className="flex justify-center items-center gap-4 w-full max-w-3xl">
        {/* Stream */}
        <motion.div layoutId="source-folder" className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[180px] shadow-sm relative">
           <Database className="size-8 text-sky-500 mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-sky-500 font-semibold mb-2">BytesIO Stream</span>
        </motion.div>
        ...
        <div className="w-32 h-6 border-y border-sky-500/20 bg-sky-500/5 relative overflow-hidden flex items-center">
           ...
           >
             <span>1011</span><span>0100</span><span>1101</span>
           </motion.div>
        </div>

        {/* Destination File */}
        <motion.div layoutId="dest-folder" className="p-6 rounded-2xl bg-violet/10 border border-violet/30 flex flex-col items-center min-w-[180px] shadow-sm">
           <FileDown className="size-8 text-violet mb-3" />
           <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-violet font-semibold mb-2">Open File</span>
           <motion.div 
             animate={{ height: [0, 40] }}
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        <code className="bg-surface-2 px-1 rounded">copyfileobj()</code> pipes raw data from any open stream (like a download or memory buffer) directly into an open file.
      </p>
    </motion.div>
  )
}
...
function Step5RmTree() {
  return (
    ...
    >
      <motion.div 
        animate={{ opacity: [1, 0.5, 1], scale: [1, 1.02, 1], borderColor: ["rgba(244,63,94,0.3)", "rgba(244,63,94,0.8)", "rgba(244,63,94,0.3)"] }}
      ...
      >
        <span className="text-rose-500 font-bold">shutil</span>.<span className="text-rose-400">rmtree</span>(workspace)
      </motion.div>
      ...
      <div className="flex justify-center items-center w-full max-w-3xl relative">
        ...
        >
           ...
           <span className="font-mono text-sm uppercase tracking-[0.2em] text-foreground font-semibold mb-6">workspace/</span>
           
           <div className="grid grid-cols-3 gap-3 w-full">
             <div className="p-3 rounded bg-surface-2 border border-hairline flex flex-col items-center gap-1"><Folder className="size-4" /><span className="text-[9px]">/data</span></div>
             <div className="p-3 rounded bg-surface-2 border border-hairline flex flex-col items-center gap-1"><FileText className="size-4" /><span className="text-[9px]">app.py</span></div>
             <div className="p-3 rounded bg-surface-2 border border-hairline flex flex-col items-center gap-1"><FileType2 className="size-4" /><span className="text-[9px]">temp.log</span></div>
           </div>
      ...
      <p className="mt-12 text-sm text-muted-foreground/80 max-w-md text-center">
        <code className="bg-surface-2 text-rose-400 px-1 rounded">rmtree()</code> recursively and permanently destroys a directory and all its contents. <strong>There is no undo.</strong>
      </p>
    </motion.div>
  )
}
