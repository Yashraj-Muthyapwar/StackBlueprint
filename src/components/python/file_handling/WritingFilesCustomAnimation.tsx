  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-rose-500/30 bg-rose-500/5 font-mono text-[13px] shadow-sm w-full max-w-lg">
        ...
        <span className="text-amber">'log.txt'</span>, <span className="text-amber">'w'</span>){" "}
        <span className="text-blue-400">as</span> file:
        <br />
      ...
      <div className="flex justify-center gap-8 lg:gap-16 w-full max-w-2xl relative">
        <div className="flex flex-col items-center gap-3">
          <div className="p-6 rounded-2xl bg-surface border border-hairline flex flex-col items-center min-w-[160px] shadow-sm relative overflow-hidden">
            ...
            <motion.div className="mt-5 flex flex-col w-full font-mono text-xs text-foreground/90 p-3 bg-surface-2 border border-hairline rounded shadow-sm text-amber">
              "New entry\n"
            </motion.div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="p-6 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex flex-col items-center min-w-[160px] shadow-sm relative">
            ...
            <div className="mt-5 w-full bg-surface border border-rose-500/30 rounded p-3 text-xs font-mono text-muted-foreground/80 leading-relaxed whitespace-pre-wrap relative overflow-hidden">
              ...
              >
                <span className="text-muted-foreground line-through decoration-rose-500">
                  Old entry 1
                </span>
                <span className="text-muted-foreground line-through decoration-rose-500">
                  Old entry 2
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Using <code className="bg-surface-2 px-1 rounded text-rose-400">'w'</code> mode will{" "}
        <strong>completely overwrite</strong> the file if it exists, erasing all old data.
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className="mb-10 p-4 rounded-xl border border-mint/30 bg-mint/5 font-mono text-[13px] shadow-sm w-full max-w-lg">
        <span className="text-blue-400">with</span> <span className="text-blue-400">open</span>(
        <span className="text-amber">'log.txt'</span>, <span className="text-amber">'a'</span>){" "}
        <span className="text-blue-400">as</span> file:
        <br />
        &nbsp;&nbsp;&nbsp;&nbsp;file.write(<span className="text-amber">'New entry\\n'</span>)
      </div>
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Using <code className="bg-surface-2 px-1 rounded text-mint">'a'</code> mode preserves the
        old data and safely adds the new data to the very end.
      </p>
    </motion.div>
  );
}
...
function Step3Newline() {
  return (
    ...
    >
      ...
      <div className="flex justify-center gap-8 lg:gap-16 w-full max-w-2xl relative">
        <div className="flex flex-col items-center gap-3">
          <div className="p-6 rounded-2xl bg-amber/10 border border-amber/20 flex flex-col items-center min-w-[200px] shadow-sm relative">
            ...
            <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-amber font-semibold">
              Result on Disk
            </span>
      ...
      <p className="mt-8 text-sm text-muted-foreground/80 max-w-sm text-center">
        Because there is no <code className="bg-surface-2 px-1 rounded text-amber">\n</code>, Python
        does not start a new line. It squashes the text together as <strong>AppleOrange</strong>.
      </p>
    </motion.div>
  );
}
