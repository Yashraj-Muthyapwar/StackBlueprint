  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-8 text-center max-w-md">
        An abstract class defines what an object must do, hiding how it actually processes the
        payment.
      </p>
      ...
      <div className="flex flex-row w-full gap-12 items-center justify-center">
        ...
        <div className="relative w-[340px] h-[340px] shrink-0">
          <motion.div
            className="absolute top-[30px] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center"
          ...
          >
            <motion.button
              animate={{
                ...
                backgroundColor: clicked ? "var(--surface-2)" : "var(--surface)",
              }}
          ...
          <svg className="absolute inset-0 w-full h-full z-10" viewBox="0 0 340 340">
            ...
            />
            <motion.path
              d="M 170,180 L 170,220 L 90,220 L 90,250"
              fill="none"
              stroke="var(--hairline)"
              strokeWidth="2"
            />
            <motion.path
              d="M 170,180 L 170,220 L 250,220 L 250,250"
              fill="none"
              stroke="var(--hairline)"
              strokeWidth="2"
            />
          ...
          <AnimatePresence>
            {clicked && (
              <motion.div
                ...
                animate={
                  method === "card"
                    ? { top: 250, left: 90, opacity: 1 }
                    : { top: 250, left: 250, opacity: 1 }
                }
                exit={{ opacity: 0 }}
          ...
          {/* Base Class */}
          <motion.div
            className="absolute top-[120px] left-1/2 -translate-x-1/2 z-20"
          ...
          {/* Subclasses */}
          <motion.div
            className="absolute top-[250px] left-[90px] -translate-x-1/2 z-20"
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            <NodeCard
              icon={CreditCard}
              title="CreditCard"
              subtitle="def pay(amount):"
              color={method === "card" && clicked ? "emerald" : "slate"}
              highlight={method === "card" && clicked}
            />
          </motion.div>

          <motion.div
            className="absolute top-[250px] left-[250px] -translate-x-1/2 z-20"
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            <NodeCard
              ...
              title="PayPal"
              subtitle="def pay(amount):"
              color={method === "paypal" && clicked ? "emerald" : "slate"}
              highlight={method === "paypal" && clicked}
            />
          </motion.div>
        </div>
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-12 text-center max-w-md">
        Inside the `CreditCard` class, processing a payment requires complex network steps. The
        Checkout interface doesn't need to know them.
      </p>
          ...
          <div className="w-56 h-40 bg-surface-2 border-2 border-hairline rounded-xl p-4 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="flex items-center justify-center h-full z-10 my-2">
              <motion.div
                ...
                className="bg-emerald-500/20 p-2 rounded-full text-emerald-500"
              >
...
function Step2Enforcement() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-8 text-center max-w-md">
        Child classes MUST implement all abstract methods before they can be used. Python enforces
        this rule.
      </p>
...
<span class="text-pink-600 dark:text-pink-400">class</span> <span class="text-amber-600 dark:text-amber-400">GiftCard</span>(<span class="text-blue-600 dark:text-blue-400">Payment</span>):<br />
          ...
          <AnimatePresence mode="wait">
            {!showFix ? (
              <motion.div
                key="badcode"
                className="text-pink-600 dark:text-pink-400 h-[40px] mt-1"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              >
              ...
              >
                &nbsp;&nbsp;&nbsp;&nbsp;
                <span className="text-pink-600 dark:text-pink-400">def</span>{" "}
                <span className="text-blue-600 dark:text-blue-400">pay</span>(self, amount):
                <br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
function Step3ABC() {
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-8 text-center max-w-md">
        Python prevents you from directly instantiating an abstract class because it's incomplete.
      </p>

      <div className="flex flex-col items-center justify-center gap-8 w-full">
        <div className="flex items-center gap-6">
          <motion.div
            className="flex items-center gap-4 bg-rose-500/10 border border-rose-500/30 p-4 rounded-xl relative overflow-hidden shadow-sm"
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-rose-500 mt-1 font-mono">
        ...
        <div
          className="w-[480px] bg-surface-2/30 rounded-lg p-5 font-mono text-sm border border-hairline text-left shadow-lg"
          dangerouslySetInnerHTML={{
            __html: `
...
function NodeCard({
  ...
  const colors = {
    emerald:
      "text-emerald-500 border-emerald-500/50 bg-emerald-500/10 shadow-[0_0_20px_rgba(16,185,129,0.2)]",
    rose: "text-rose-500 border-rose-500/50 bg-rose-500/10 shadow-[0_0_20px_rgba(244,63,94,0.2)]",
    indigo:
      "text-indigo-500 border-indigo-500/50 bg-indigo-500/10 shadow-[0_0_20px_rgba(99,102,241,0.2)]",
  ...
  return (
    <div
      className={cn(
        `rounded-2xl border-2 flex flex-col items-center justify-center backdrop-blur-md relative overflow-hidden text-center transition-all duration-300`,
        colors[color],
        "w-36 p-4 gap-2",
        highlight && "scale-105",
      )}
    >
      ...
      {subtitle && (
        <span className="text-[10px] font-mono opacity-80 leading-tight relative z-10">
          {subtitle}
        </span>
      )}
    </div>
