  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-12 text-center max-w-md">
        Inheritance forces objects into rigid family trees. Composition builds objects out of
        smaller parts.
      </p>
      ...
      <div className="flex flex-row w-full gap-16 items-center justify-center">
        ...
        <div className="flex flex-col items-center p-6 border-2 border-hairline bg-surface-2/30 rounded-2xl relative w-64 h-[280px]">
          <div className="absolute -top-3 bg-indigo-500/10 text-indigo-500 border border-indigo-500/30 px-3 py-1 rounded-full text-[10px] font-mono font-bold flex items-center gap-1">
            <GitBranch className="size-3" /> Inheritance
          </div>
          ...
          <div className="mt-4 flex flex-col items-center">
            ...
            <div className="h-10 w-[2px] bg-indigo-500/30 flex items-center justify-center relative">
              <div className="absolute bg-indigo-500 text-white px-2 py-0.5 text-[9px] font-mono font-bold rounded-full whitespace-nowrap z-10 shadow-sm">
                IS A
              </div>
            </div>
        ...
        <div className="flex flex-col items-center p-6 border-2 border-emerald-500/30 bg-emerald-500/5 rounded-2xl relative w-64 h-[280px] shadow-[0_0_30px_rgba(16,185,129,0.1)]">
          <div className="absolute -top-3 bg-emerald-500 text-white px-3 py-1 rounded-full text-[10px] font-mono font-bold flex items-center gap-1 shadow-sm">
            <Combine className="size-3" /> Composition
          </div>
                ...
                <div className="flex flex-col gap-2 w-full mt-2">
                  <div className="flex items-center justify-between bg-surface-2 px-3 py-1.5 rounded-lg border border-hairline w-full">
                    <span className="text-[9px] font-mono font-bold text-muted-foreground flex items-center gap-1">
                      <div className="size-1.5 rounded-full bg-emerald-500" /> HAS A
                    </span>
                    <div className="flex items-center gap-1 text-emerald-500">
                      <Crosshair className="size-3" />{" "}
                      <span className="text-[10px] font-mono font-bold">Weapon</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-surface-2 px-3 py-1.5 rounded-lg border border-hairline w-full">
                    ...
                    <div className="flex items-center gap-1 text-emerald-500">
                      ...
                      <span className="text-[10px] font-mono font-bold">Battery</span>
                    </div>
                  </div>
                </div>
              </div>
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-8 text-center max-w-md">
        You build the Robot by passing smaller, independent objects into it.
      </p>
      ...
      <div className="flex flex-col items-center gap-8 w-full mt-8">
        <div className="flex items-center justify-center gap-8 w-full relative h-[180px]">
          ...
          <div className="flex flex-col gap-4 absolute left-10">
            <motion.div
              animate={
                phase >= 1
                  ? { x: 160, y: 25, scale: 0.8, opacity: 0 }
            ...
            <motion.div
              animate={
                phase >= 2
                  ? { x: 160, y: -45, scale: 0.8, opacity: 0 }
                  : { x: 0, y: 0, scale: 1, opacity: 1 }
              }
              transition={{ duration: 0.8, ease: "backIn" }}
          ...
          <div className="w-56 h-48 border-4 border-dashed border-emerald-500/30 rounded-2xl p-4 flex flex-col justify-between absolute z-10 bg-surface/50 backdrop-blur-sm">
            ...
            <div className="flex flex-col gap-2 w-full mt-2">
              <div
                className={cn(
                  "h-12 w-full rounded-xl border-2 flex items-center justify-center transition-all duration-300",
                  phase >= 1
                    ? "bg-amber-500/10 border-amber-500"
                    : "bg-surface-2 border-hairline border-dashed",
                )}
              >
                {phase >= 1 ? (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="flex items-center gap-2 text-amber-500 font-mono text-[10px] font-bold"
                  >
              ...
              >
                {phase >= 2 ? (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="flex items-center gap-2 text-rose-500 font-mono text-[10px] font-bold"
                  >
        ...
        <div className="w-[480px] bg-surface-2/30 rounded-lg p-5 font-mono text-sm border border-hairline text-left shadow-lg mt-4 flex flex-col items-center">
          <div className="bg-black/5 dark:bg-white/5 px-4 py-2 rounded-lg border border-hairline text-center w-full">
            <span className="text-pink-600 dark:text-pink-400">my_robot</span> ={" "}
            <span className="text-amber-600 dark:text-amber-400">Robot</span>(
            <span className="text-emerald-600 dark:text-emerald-400">power</span>=
            <span className="text-amber-600 dark:text-amber-400">Battery</span>(),{" "}
            <span className="text-emerald-600 dark:text-emerald-400">weapon</span>=
            <span className="text-amber-600 dark:text-amber-400">Laser</span>())
          </div>
        </div>
      </div>
    </motion.div>
...
function Step2Delegation() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-12 text-center max-w-md">
        The Robot doesn't know how to shoot. When you call `attack()`, it passes the job to its
        weapon.
      </p>
      ...
      <div className="flex items-center gap-16 justify-center w-full">
        <div className="flex flex-col items-center gap-4">
          <motion.button
            animate={{
              scale: attacking ? 0.95 : 1,
              backgroundColor: attacking ? "rgba(239, 68, 68, 0.1)" : "var(--surface)",
            }}
          ...
          <div className="w-56 h-48 border-2 border-emerald-500 rounded-2xl p-4 flex flex-col justify-between relative bg-emerald-500/5">
            <motion.div
              animate={{ opacity: attacking ? [0, 0.5, 0] : 0 }}
              transition={{ duration: 0.5 }}
              className="absolute inset-0 bg-rose-500/10 rounded-2xl"
            />
...
function Step3Flexibility() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-8 text-center max-w-md">
        You can swap out components at runtime without changing the Robot class at all.
      </p>
            ...
            <AnimatePresence mode="wait">
              {weapon === "laser" ? (
                <motion.div
                  ...
                  className="h-20 w-full rounded-xl border-2 bg-rose-500/10 border-rose-500 flex items-center justify-center shadow-sm"
                >
                  <div className="flex items-center gap-2 text-rose-500 font-mono text-[10px] font-bold">
                ...
                <motion.div
                  ...
                  animate={{ scale: 1, opacity: 1 }}
                  exit={{ scale: 0.8, opacity: 0 }}
                  className="h-20 w-full rounded-xl border-2 bg-blue-500/10 border-blue-500 flex items-center justify-center shadow-sm"
                >
            ...
            <button
              ...
              className={cn(
                ...
                weapon === "laser"
                  ...
                  : "bg-transparent border-transparent text-muted-foreground hover:bg-surface-2",
              )}
            >
