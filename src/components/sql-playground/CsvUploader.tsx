...
import { Upload, X, FileType, Check, AlertTriangle, Database as DatabaseIcon, Loader2 } from "lucide-react";
...
export function CsvUploader({ onUpload, onClose, currentDataset }: CsvUploaderProps) {
  ...
  const processDroppedFile = async (dropped: File) => {
    ...
    if (!isSqlite) {
      ...
      let inferred = dropped.name.replace(/\.csv$/i, "").replace(/[^a-zA-Z0-9_]/g, "_").toLowerCase();
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (dropped) {
      const isOk = dropped.name.endsWith('.csv') || dropped.name.endsWith('.db') || dropped.name.endsWith('.sqlite');
      if (isOk) {
    ...
    <div className="sqlx-modal-overlay">
      <div className="sqlx-modal">
        <div className="sqlx-modal-header">
          <h2>Upload Data</h2>
          <button onClick={onClose} disabled={uploading} className="sqlx-icon-btn"><X size={18} /></button>
        </div>
          ...
          {!file ? (
            ...
            <div className="sqlx-upload-form">
              <div className="sqlx-file-info">
                ...
                <button className="sqlx-icon-btn" onClick={() => setFile(null)} disabled={uploading}>
                  <X size={16} />
                </button>
              </div>
              ...
              {mode === "csv" ? (
                ...
                <div className="sqlx-field">
                  <label>Select Tables ({selectedTables.size} / {availableTables.length})</label>
                ...
                <div className="sqlx-radio-group">
                  ...
                  <label className={`sqlx-radio ${target === 'current' ? 'selected' : ''}`}>
                    ...
                    <div className="content">
                      ...
                      <span>Imports into an <code>uploads</code> schema alongside the {currentDataset} course dataset.</span>
                    </div>
                  </label>
                </div>
              </div>
              ...
              {target === 'current' && (
                <div className="sqlx-alert">
                  ...
                  <span>Course tables are available only for this temporary session. Uploaded data remains safely separated in the <code>uploads</code> schema.</span>
                </div>
              )}
            </div>
          )}
        ...
        {file && !inspecting && (
          <div className="sqlx-modal-footer">
            <button className="sqlx-btn" onClick={onClose} disabled={uploading}>Cancel</button>
