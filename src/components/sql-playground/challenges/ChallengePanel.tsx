...
export function ChallengePanel({
  ...
  const starterSql =
    ...
    `SELECT * FROM ${challenge.requiredTables[0] || "table_name"} LIMIT 10;`;
  ...
  return (
    <section
      className="sqlx-editor-card"
      style={{ flex: "none", border: "1px solid var(--sqlx-line)" }}
    >
      <div
        className="sqlx-toolbar"
        style={{
          padding: "8px 12px",
          borderBottom: "1px solid var(--sqlx-line)",
          backgroundColor: "var(--sqlx-panel-2)",
        }}
      >
      ...
      <div style={{ padding: "12px 16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "16px" }}>
          ...
          {hasHints && (
            <button className="sqlx-btn" onClick={revealHint} disabled={allHintsRevealed}>
              <Lightbulb size={13} />
              {hintIndex === -1 ? "Hint" : allHintsRevealed ? "No more hints" : "Next hint"}
            </button>
          )}

          <button className="sqlx-btn" onClick={() => setShowSolution(!showSolution)}>
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: "20px" }}>
              {challenge.hints.slice(0, hintIndex + 1).map((hint, i) => (
                <li key={i} style={{ marginBottom: i < hintIndex ? "4px" : 0 }}>
                  {hint}
                </li>
              ))}
            </ul>
            ...
            <div style={{ fontSize: "13px" }}>
              {isPassed && (
                <>
                  ...
                  {challenge.success.nextConcept && onNextChallenge && (
                    <div style={{ marginTop: "8px" }}>Next: {challenge.success.nextConcept}.</div>
                  )}
                </>
              )}
              ...
              {feedback.state === "unavailable" && (
                <>
                  ...
                  <div style={{ color: "#991b1b" }}>{feedback.message}</div>
                </>
              )}
            </div>
          </div>
        ...
        {!feedback && (
          ...
          >
            Write your query, then choose <strong>Run and check</strong>. We check the result, not
