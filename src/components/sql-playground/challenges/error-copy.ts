  ...
  switch (category) {
    ...
    case "table-missing": {
      ...
      return {
        ...
        body: tables
          ? `Try using one of this challenge's required tables: ${tables}.`
...
export function getCheckFeedback(reason: CheckReason, detail?: string): { body: string } {
  switch (reason) {
    ...
    case "missing-column":
      return {
        body: detail
          ? `Not quite yet. Check the required columns. ${detail} is incorrect.`
          : `Not quite yet. Make sure you return exactly the requested columns.`,
      };
    case "missing-rows":
      return {
        body: "Your query ran, but it leaves out some expected rows. Check your JOIN conditions and WHERE filters.",
      };
    case "extra-rows":
      return {
        body: "Your query includes extra rows that shouldn't be there. Look at the condition that identifies matches.",
      };
    case "wrong-order":
      return { body: "The rows are right. Now sort them as requested in the prompt." };
    case "zero-rows":
      return {
        body: "Your query ran but returned no rows. Recheck the filter and whether NULL is tested with IS NULL.",
      };
    case "aggregate-mismatch":
      return {
        body: "The grouping is close, but the totals do not match. Check the join grain before summing values.",
      };
    case "duplicate-rows":
      return {
        body: "Some rows appear more than once. A join may be multiplying records; inspect the join keys and grouping.",
      };
    case "wrong-value":
      return {
        body: "The result values don't quite match the expected output. Double check your logic.",
      };
    default:
...
export function categorizeEngineError(errorMsg: string): {
  category: QueryErrorCategory;
  detail?: string;
} {
  ...
  if (msg.includes("syntax error")) {
    const match =
      msg.match(/syntax error at or near "([^"]+)"/i) ||
      msg.match(/syntax error near "([^"]+)"/i) ||
      msg.match(/syntax error near ([^\s]+)/i);
    return { category: "syntax", detail: match ? match[1] : undefined };
  }

  if (
    (msg.includes("relation") && msg.includes("does not exist")) ||
    (msg.includes("catalog error") && msg.includes("table with name"))
  ) {
  ...
  if (
    ...
    (msg.includes("column reference") && msg.includes("is ambiguous"))
  ) {
    return { category: "ambiguous-column" };
  }

  if (
    msg.includes("must appear in the group by") ||
    msg.includes("not found in from clause") ||
    (msg.includes("aggregate") && msg.includes("group by"))
  ) {
