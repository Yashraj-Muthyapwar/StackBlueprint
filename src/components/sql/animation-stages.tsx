...
const COMMENT_STUDENTS: Row[] = [
  r(1, 1, "Ada",   "A"),
  r(2, 2, "Linus", "B"),
  r(3, 3, "Grace", "A"),
];

const commentsStages: Stage[] = [
  {
  ...
  },
  {
    ...
    steps: [
      st([0], "pending",
        ...
        { noteTone: "violet" }),
      st([0, 1], "kept",
        "Same thing on line 1. Final executed query: `SELECT * FROM Students;`",
        { noteTone: "mint" }),
    ],
  },
  {
...
const operatorsStages: Stage[] = [
  {
    ...
    steps: [
      ...
      st([2, 6], "kept",
        ...
        {
          rowsOverride: OP_ROWS.map((row) => r(row.key, row.cells[1], row.cells[2], Number(row.cells[2]) + 100)),
      ...
      st([2, 3, 4, 5, 6], "kept",
        "All four arithmetic operators evaluated together — one row in, four derived columns out.",
        {
          ...
          colsOverride: ["item", "amount", "total_amount", "offer_price", "double_price", "half_price"],
          noteTone: "violet",
        }),
      st([], "kept",
  ...
  },
  {
    ...
    steps: [
      st([2], pass((row) => Number(row.cells[2]) > 50),
        ...
        { highlightCols: [2] }),
      st([2, 3], pass((row) => Number(row.cells[2]) > 50 && row.cells[3] === 4),
  ...
  },
  {
    ...
    table: { name: "Orders", cols: OP_COLS, rows: OP_ROWS },
    steps: [
      st([2], pass((row) => Number(row.cells[2]) < 20),
        "`amount < 20` alone — only Cable qualifies.",
        { highlightCols: [2] }),
      st([2, 3], pass((row) => Number(row.cells[2]) < 20 || Number(row.cells[2]) > 300),
  ...
  },
  {
    ...
    steps: [
      st([2], pass((row) => row.cells[3] !== 4),
        ...
        { highlightCols: [3] }),
      st([2, 3], pass((row) => row.cells[3] !== 4 && Number(row.cells[2]) >= 50),
