...
export const olistOverlappingRangesStages: Stage[] = [
  {
    ...
    steps: [
      st(
        [5],
        "kept",
        "The WHERE clause selects one real Olist order. Its delivery and estimated dates are the endpoints for the two ranges.",
      ),
      st(
        [0, 1, 2, 3],
        "kept",
        "Adding one day creates [2017-10-02, 2017-10-11) and [2017-10-02, 2017-10-19). Each range includes its final business date but excludes the following day.",
        {
          rowsOverride: [r(1, "[2017-10-02, 2017-10-11)", "[2017-10-02, 2017-10-19)")],
          colsOverride: ["actual_window", "estimated_window"],
          highlightCols: [0, 1],
        },
      ),
    ],
  },
  {
    ...
    table: {
      name: "delivery ranges",
      cols: ["actual window", "estimated window"],
      rows: [r(1, "[2017-10-02, 2017-10-11)", "[2017-10-02, 2017-10-19)")],
    },
    steps: [
      st(
        [0, 1, 2, 3, 4],
        "kept",
        "The CTE creates the two real range values. No relationship has been tested yet.",
      ),
      st(
        [5, 6, 7],
        "kept",
        "The outer SELECT tests the relationship between those two values. The windows overlap, and the actual range contains October 8.",
        {
          rowsOverride: [r(1, "true", "true")],
          colsOverride: ["overlaps", "contains_date"],
          highlightCols: [0, 1],
        },
      ),
    ],
  },
  {
    ...
    table: {
      name: "orders",
      cols: ["delivered date", "estimated date"],
      rows: [r(1, "2017-10-10", "2017-10-18")],
    },
    steps: [
      st(
        [3],
        "kept",
        "The range representation is useful for intersections; this WHERE clause selects the same real Olist order for an endpoint comparison.",
      ),
      st(
        [0, 1],
        "kept",
        "October 10 is not after October 18, so the direct comparison returns false for delivered_late.",
        { rowsOverride: [r(1, "false")], colsOverride: ["delivered_late"], highlightCols: [0] },
      ),
    ],
  },
];
