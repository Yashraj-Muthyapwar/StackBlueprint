...
import { Database, MonitorSmartphone, CheckCircle2, ShieldCheck, Zap, ArrowRight, Save } from "lucide-react";
...
function Packet({ 
  ...
  const colorMap = {
    ...
    "muted-foreground": "border-muted-foreground/30 bg-muted-foreground/10 text-muted-foreground shadow-[0_0_12px_rgba(148,163,184,0.8)]",
  };
      ...
      <div className="w-full overflow-x-auto hide-scrollbar">
        <div className="relative h-[280px] min-w-[700px] bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] overflow-hidden">
          ...
          <svg className="absolute inset-0 h-full w-full pointer-events-none z-10">
            ...
            <line x1="65%" y1="70%" x2="85%" y2="70%" className="stroke-border/50 stroke-2" strokeDasharray="6 6" />
          </svg>
            ...
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl border-2 border-muted-foreground/20 bg-background shadow-lg relative">
              ...
              <div className="absolute -bottom-10 w-28 bg-background border border-hairline rounded-md p-1.5 shadow-sm flex items-center justify-center gap-2">
                {((mode === "STRONG" && step >= 2) || (mode === "EVENTUAL" && step >= 1)) ? (
                  <span className="text-[9px] font-mono font-bold text-mint flex items-center gap-1"><CheckCircle2 className="size-3" /> Unblocked</span>
                ) : (step > 0) ? (
                  <span className="text-[9px] font-mono font-medium text-amber flex items-center gap-1"><div className="size-2 rounded-full border border-amber border-t-transparent animate-spin" /> Blocked</span>
                ) : (
          ...
          <div className="absolute left-[50%] top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] flex flex-col items-center gap-3 z-30">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl border-2 border-mint/40 bg-background shadow-lg relative">
              ...
              <div className="absolute -bottom-10 w-28 bg-background border border-hairline rounded-md p-1.5 shadow-sm flex items-center justify-center gap-2">
                 <span className="text-[9px] font-mono font-medium text-muted-foreground flex items-center gap-1"><Save className="size-3 text-mint" /> LEADER</span>
              </div>
            </div>
          </div>
          ...
          {['A', 'B'].map((id, index) => (
            ...
            >
              ...
              <span className="font-mono text-[10px] font-bold text-muted-foreground">FOLLOWER {id}</span>
            </div>
          ))}
          ...
          <AnimatePresence mode="wait">
            {mode === "STRONG" && step === 0 && (
               <Packet key="s0" label={<>Write <ArrowRight className="ml-1 size-3" /></>} color="blue" pathX={["calc(15% + 60px)", "calc(50% - 60px)"]} pathY={["50%", "50%"]} pathTimes={[0, 1]} duration={4.5} />
            )}
            {mode === "STRONG" && step === 1 && (
              <>
                <Packet key="s1a" label="Replicate" color="violet" pathX={["calc(50% + 60px)", "65%", "65%", "calc(85% - 60px)"]} pathY={["50%", "50%", "30%", "30%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
                <Packet key="s1b" label="Replicate" color="violet" pathX={["calc(50% + 60px)", "65%", "65%", "calc(85% - 60px)"]} pathY={["50%", "50%", "70%", "70%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
              </>
            )}
            {mode === "STRONG" && step === 2 && (
              <>
                <Packet key="s2a" label={<>ACK <CheckCircle2 className="ml-1 size-3" /></>} color="mint" pathX={["calc(85% - 60px)", "65%", "65%", "calc(50% + 60px)"]} pathY={["30%", "30%", "50%", "50%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
                <Packet key="s2b" label={<>ACK <CheckCircle2 className="ml-1 size-3" /></>} color="mint" pathX={["calc(85% - 60px)", "65%", "65%", "calc(50% + 60px)"]} pathY={["70%", "70%", "50%", "50%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
                
                {/* Final ACK back to Client */}
                <Packet key="s2c" label={<>Success <CheckCircle2 className="ml-1 size-3" /></>} color="mint" pathX={["calc(50% - 60px)", "calc(15% + 60px)"]} pathY={["50%", "50%"]} pathTimes={[0, 1]} duration={4.5} />
              </>
            )}

            {mode === "EVENTUAL" && step === 0 && (
               <Packet key="e0" label={<>Write <ArrowRight className="ml-1 size-3" /></>} color="blue" pathX={["calc(15% + 60px)", "calc(50% - 60px)"]} pathY={["50%", "50%"]} pathTimes={[0, 1]} duration={4.5} />
            )}
            {mode === "EVENTUAL" && step === 1 && (
              <>
                {/* Instant ACK to client */}
                <Packet key="e1-ack" label={<>Success <CheckCircle2 className="ml-1 size-3" /></>} color="mint" pathX={["calc(50% - 60px)", "calc(15% + 60px)"]} pathY={["50%", "50%"]} pathTimes={[0, 1]} duration={4.5} />
                
                {/* Background Sync */}
                <Packet key="e1-repA" label="Async Sync" color="amber" pathX={["calc(50% + 60px)", "65%", "65%", "calc(85% - 60px)"]} pathY={["50%", "50%", "30%", "30%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
                <Packet key="e1-repB" label="Async Sync" color="amber" pathX={["calc(50% + 60px)", "65%", "65%", "calc(85% - 60px)"]} pathY={["50%", "50%", "70%", "70%"]} pathTimes={[0, 0.35, 0.65, 1]} duration={6} />
              </>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex-1 space-y-2">
            <h3 className="text-sm font-bold flex items-center gap-2 text-foreground">
              {mode === "STRONG" ? <ShieldCheck className="size-4 text-amber" /> : <Zap className="size-4 text-mint" />}
          ...
          <div className="flex-1 space-y-2 w-full">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground font-mono bg-background p-2 rounded-md border border-hairline min-h-[60px] overflow-y-auto max-h-[80px]">
              {mode === "STRONG" && step === 0 && "Step 1: Client sends a Write request to the Leader."}
              {mode === "STRONG" && step === 1 && "Step 2: Leader forwards the Write to all Followers. Client is BLOCKED."}
              {mode === "STRONG" && step === 2 && "Step 3: Followers write the data and send an ACK back to the Leader. Leader replies Success to Client."}
              {mode === "STRONG" && step >= 3 && "Step 4: Complete. High Consistency, High Latency."}
              
              {mode === "EVENTUAL" && step === 0 && "Step 1: Client sends a Write request to the Leader."}
              {mode === "EVENTUAL" && step === 1 && "Step 2: Leader instantly replies Success to Client. Client is UNBLOCKED. Simultaneously, Leader starts background async sync to Followers."}
              {mode === "EVENTUAL" && step >= 2 && "Step 3: Complete. Low Latency, Eventual Consistency."}
            </p>
          </div>
        </div>
      </div>
