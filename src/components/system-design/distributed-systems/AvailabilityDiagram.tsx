        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 mr-4 hidden md:flex">
            ...
            <span className="font-mono text-xs text-muted-foreground">
              Uptime: <span className="text-mint font-bold">{getPercentage()}</span>
            </span>
          </div>
          <button
        ...
        <div className="relative h-[340px] min-w-[700px] bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] overflow-hidden">
          ...
          <div className="absolute left-[50%] top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] flex flex-col items-center gap-3 z-30">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl border-2 border-mint/40 bg-background shadow-lg relative">
              ...
              <div className="absolute -bottom-10 w-28 bg-background border border-hairline rounded-md p-1.5 shadow-sm flex items-center justify-center gap-2">
                <span className="text-[9px] font-mono font-medium text-muted-foreground flex items-center gap-1">
                  <HeartPulse
                    className={`size-3 ${step === 4 ? "text-rose-500 animate-pulse" : "text-mint"}`}
                  />
                  Health Check
                </span>
              </div>
          ...
          {["A", "B", "C"].map((id, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl border-2 shadow-lg relative transition-colors duration-500 ${isHealthy ? "border-muted-foreground/20 bg-background" : "border-rose-500/40 bg-rose-500/10"}`}
                >
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex-1 space-y-2">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground min-h-[60px]">
              When users visit a website, they don't connect directly to a server. They connect to a{" "}
              <strong>Load Balancer</strong>. The Load Balancer constantly pings the servers (Health
              Checks) to make sure they are alive. If a server crashes, the Load Balancer instantly
              detects it and stops sending traffic there, maintaining high availability for the
              users.
            </p>
          </div>
          ...
          <div className="flex-1 space-y-2 w-full">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground font-mono bg-background p-2 rounded-md border border-hairline min-h-[60px] overflow-y-auto max-h-[80px]">
              {step === 0 &&
                "Step 1: Client sends Request 1. Load Balancer receives it and routes it to Server A."}
              {step === 1 &&
                "Step 2: Client sends Request 2. Load Balancer routes it to Server B (Round Robin)."}
              {step === 2 && "Step 3: Server B crashes unexpectedly!"}
              {step === 3 &&
                "Step 4: Client sends Request 3. Load Balancer routes it to Server B. The request drops!"}
              {step === 4 &&
                "Step 5: Load Balancer performs a Health Check and marks Server B as dead."}
              {step === 5 &&
                "Step 6: Client sends Request 4. Load Balancer safely routes it to Server C, skipping B."}
              {step === 6 &&
                "Step 7: Client sends Request 5. Load Balancer routes it back to Server A."}
              {step >= 7 &&
                "Animation Complete. System is operating at degraded capacity (99.99%) but remains available."}
            </p>
          </div>
        </div>
      </div>
