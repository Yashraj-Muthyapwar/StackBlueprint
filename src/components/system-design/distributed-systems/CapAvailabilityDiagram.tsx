    ...
    <div className="flex flex-col rounded-xl border border-hairline bg-card/50 overflow-hidden shadow-sm my-6">
      ...
      <div className="relative h-[240px] w-full bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] overflow-hidden">
        ...
        <div className="absolute left-12 top-1/2 flex -translate-y-1/2 flex-col items-center gap-2 z-20">
          ...
          <div
            className={`flex flex-col w-28 items-center justify-center rounded-xl border-2 border-muted-foreground/20 bg-background shadow-lg overflow-hidden`}
          >
            ...
            <div className="py-2 w-full flex flex-col items-center">
              <span className="text-xs font-mono font-medium flex items-center gap-1">
                <Heart className="size-3 text-rose-500" /> {likesA}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="absolute right-12 top-1/2 flex -translate-y-1/2 flex-col items-center gap-2 z-20">
          ...
          >
            ...
            <div className="py-2 w-full flex flex-col items-center">
              <span className="text-xs font-mono font-medium flex items-center gap-1">
                <Heart className="size-3 text-rose-500" /> {likesB}
              </span>
            </div>
          </div>
        </div>
          ...
          <AnimatePresence>
            {step === 1 && (
              <Packet
                ...
                label={
                  <>
                    Get Feed <ArrowRight className="ml-1 size-3" />
                  </>
                }
                color="blue"
                fromX="50%"
            ...
            {step === 2 && (
              <Packet
                ...
                label={
                  <>
                    100 Likes <CheckCircle2 className="ml-1 size-3" />
                  </>
                }
                color="mint"
      ...
      <div className="border-t border-hairline bg-muted/20 p-4">
        <p className="text-xs leading-relaxed text-muted-foreground">
          <strong className="text-foreground">Availability</strong> means every request receives a
          fast response, even if nodes aren't perfectly synchronized yet. Here, the client gets a
          quick answer from Node B. Even though the data is slightly stale (100 vs 150), the system
          prioritized answering the user over waiting for the background sync to finish.
        </p>
      </div>
    </div>
  );
