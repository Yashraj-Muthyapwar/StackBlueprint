    ...
    <div className="flex flex-col rounded-xl border border-hairline bg-card/50 overflow-hidden shadow-sm my-6">
      <div className="flex items-center justify-between border-b border-hairline bg-muted/20 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Partition Tolerance Demo</span>
        </div>
        <button
      ...
      <div className="relative h-[200px] w-full bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] overflow-hidden flex items-center justify-center gap-12">
        ...
        <div className="absolute top-1/2 left-24 right-24 h-[2px] -translate-y-1/2 flex">
           <div className={`h-full flex-1 border-t-2 border-dashed transition-colors duration-500 ${networkPartition ? 'border-rose-500/20' : 'border-border/50'}`} />
        ...
        <div className="flex flex-col items-center gap-2 z-20">
          <span className="font-mono text-[10px] font-bold text-muted-foreground">Node A (Alive)</span>
        ...
        <AnimatePresence>
          {networkPartition && (
            ...
            >
              ...
              <span className="absolute -bottom-6 font-mono text-[10px] text-rose-500 font-bold tracking-wider">NETWORK CUT</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {/* Node B */}
        <div className="flex flex-col items-center gap-2 z-20">
          <span className="font-mono text-[10px] font-bold text-muted-foreground">Node B (Alive)</span>
          <div className="flex flex-col w-24 items-center justify-center rounded-xl border-2 border-mint/40 bg-mint/5 shadow-lg overflow-hidden">
            <div className="w-full bg-muted/30 py-3 flex justify-center border-b border-hairline relative">
              <Server className="size-6 text-mint" />
              <div className="absolute top-2 right-2 size-2 rounded-full bg-mint animate-pulse" />
      ...
      <div className="border-t border-hairline bg-muted/20 p-4">
        <p className="text-xs leading-relaxed text-muted-foreground">
          <strong className="text-foreground">Partition Tolerance (P)</strong> means the system continues operating despite network communication failures. 
          Notice that Node A and Node B are both completely healthy and running, but the cable connecting them has been cut. Distributed systems must assume partitions will eventually happen.
        </p>
      </div>
    </div>
  );
