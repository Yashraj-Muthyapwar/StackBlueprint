...
export function CapTheoremDiagram() {
  ...
  const handleBankAction = (node: "A" | "B") => {
    ...
    if (networkPartition) {
      setBankError(
        `Node ${node} rejected transaction: Cannot safely coordinate with other nodes (CP mode).`,
      );
      return;
    }
        ...
        <AnimatePresence>
          {networkPartition && (
            ...
            >
              ...
              <span className="absolute -bottom-8 font-mono text-[10px] text-rose-500 font-bold tracking-wider">
                PARTITION
              </span>
            </motion.div>
          )}
            ...
            <div className="p-4 w-full flex flex-col items-center gap-2">
              {scenario === "CP" ? (
                <>
                  ...
                  <button
                    onClick={() => handleBankAction("A")}
                    className="mt-2 w-full px-2 py-1.5 text-[10px] font-bold rounded bg-foreground text-background hover:bg-foreground/90 transition-colors"
                  >
                    Withdraw $200
                  </button>
                </>
                ...
                <>
                  <div className="text-sm font-mono font-medium flex items-center gap-1">
                    <Heart className="size-4 text-rose-500" /> {likesA}
                  </div>
                  {pendingSyncA > 0 && (
                    <span className="text-[9px] text-amber animate-pulse">
                      +{pendingSyncA} pending sync
                    </span>
                  )}
                  <button
                    onClick={() => handleSocialAction("A")}
                    className="mt-2 w-full px-2 py-1.5 text-[10px] font-bold rounded bg-foreground text-background hover:bg-foreground/90 transition-colors"
                  >
                    Like Post
                  </button>
          ...
          <div className="flex flex-col w-40 items-center justify-center rounded-xl border-2 border-muted-foreground/20 bg-background shadow-lg overflow-hidden">
            ...
            <div className="p-4 w-full flex flex-col items-center gap-2">
              {scenario === "CP" ? (
                ...
                <>
                  <div className="text-sm font-mono font-medium flex items-center gap-1">
                    <Heart className="size-4 text-rose-500" /> {likesB}
                  </div>
                  {pendingSyncB > 0 && (
                    <span className="text-[9px] text-amber animate-pulse">
                      +{pendingSyncB} pending sync
                    </span>
                  )}
                  <button
                    onClick={() => handleSocialAction("B")}
                    className="mt-2 w-full px-2 py-1.5 text-[10px] font-bold rounded bg-foreground text-background hover:bg-foreground/90 transition-colors"
                  >
                    Like Post
                  </button>
      ...
      <div className="border-t border-hairline bg-muted/20 p-4">
        <div className="flex flex-col gap-2 items-start">
          <h3 className="text-sm font-bold flex items-center gap-2 text-foreground">
            {scenario === "CP" ? (
              <Building className="size-4 text-amber" />
            ) : (
              <Heart className="size-4 text-mint" />
            )}
            {scenario === "CP"
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            {scenario === "CP" ? (
              <>
                In a financial system, <strong className="text-foreground">Consistency</strong> is
                critical. When the network is healthy, withdrawing $200 syncs to both nodes. When a{" "}
                <strong>Partition</strong> occurs, the nodes cannot safely communicate. To prevent
                you from withdrawing the same $200 twice, the system{" "}
                <strong>rejects the operation</strong>, choosing Consistency (safe data) over
                Availability (serving the request).
              </>
            ) : (
              <>
                In a social media feed, <strong className="text-foreground">Availability</strong> is
                more important than immediate consistency. When a <strong>Partition</strong> occurs,
                both nodes continue to accept "Likes" to keep the app working for users. This
                creates <strong>temporary stale data</strong> (divergence) between the nodes. When
                the partition is resolved, the nodes reconcile their differences (Eventual
                Consistency).
              </>
            )}
          </p>
