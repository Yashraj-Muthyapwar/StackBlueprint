        ...
        <div className="relative h-[340px] min-w-[700px] bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] overflow-hidden">
          ...
          <svg className="absolute inset-0 h-full w-full pointer-events-none z-10">
            ...
            <line
              ...
              x2="85%"
              y2="70%"
              className="stroke-border/50 stroke-2 opacity-60"
              strokeDasharray="6 6"
            />
          </svg>
          ...
          <div className="absolute left-[50%] top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] flex flex-col items-center gap-3 z-20">
            ...
            <span className="font-mono text-xs font-bold text-foreground mt-8 text-center bg-background px-2 py-0.5 rounded border border-hairline shadow-sm">
              SERVICE A
            </span>
          </div>
          ...
          <div className="absolute left-[75%] top-[30%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center w-[120px] z-30">
            ...
            <div className="mt-8 flex flex-col items-center">
              ...
              >
                <Zap
                  className={`size-3.5 ${cbState === "OPEN" ? "text-rose-500" : cbState === "HALF_OPEN" ? "text-amber" : "text-mint"}`}
                />
                <span
                  className={`text-[10px] font-mono font-bold ${cbState === "OPEN" ? "text-rose-500" : cbState === "HALF_OPEN" ? "text-amber" : "text-mint"}`}
                >
                  {cbState}
                </span>
              </motion.div>
          ...
          <div className="absolute left-[85%] top-[30%] -translate-x-1/2 -translate-y-1/2 w-[120px] flex flex-col items-center gap-3 z-20">
            <motion.div
              animate={{
                borderColor:
                  serviceBHealth === "FAILING" ? "rgba(244,63,94,0.4)" : "rgba(16,185,129,0.4)",
                backgroundColor:
                  serviceBHealth === "FAILING" ? "rgba(244,63,94,0.1)" : "rgba(16,185,129,0.05)",
              }}
            ...
            <span className="font-mono text-xs font-bold text-foreground mt-8 text-center bg-background px-2 py-0.5 rounded shadow-sm border border-hairline">
              SERVICE B
            </span>
          </div>
          ...
          <div className="absolute left-[85%] top-[70%] -translate-x-1/2 -translate-y-1/2 w-[120px] flex flex-col items-center gap-3 z-20">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border-2 border-amber/40 border-dashed bg-background shadow-lg relative opacity-80">
              <RefreshCcw
                className={`size-6 ${cbState === "OPEN" ? "text-amber animate-spin-slow" : "text-muted-foreground"}`}
              />
            </div>
            <span className="font-mono text-[10px] font-bold text-muted-foreground text-center tracking-wider">
              CACHE
            </span>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {(step === 3 || step === 4) && (
              <Packet
                ...
                label={
                  <>
                    Timeout <Clock className="ml-1 size-3" />
                  </>
                }
                color="rose"
                pathX={getPath("B").x}
            ...
            {(step === 5 || step === 6) && (
              <Packet
                ...
                label={
                  <>
                    Fallback <AlertTriangle className="ml-1 size-3" />
                  </>
                }
                color="amber"
                pathX={getPath("CACHE").x}
                pathY={getPath("CACHE").y}
                pathTimes={getPath("CACHE").times}
                duration={6}
              />
            )}
            ...
            {step === 7 && (
              <Packet
                ...
                label={
                  <>
                    Test <Settings className="ml-1 size-3 animate-spin" />
                  </>
                }
                color="mint"
                pathX={getPath("B").x}
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex-1 space-y-2">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground min-h-[60px]">
              If Service B goes down, Service A will start experiencing timeouts. Instead of
              continuing to hammer a dead service (and exhausting its own resources waiting), the{" "}
              <strong>Circuit Breaker</strong> trips OPEN. It immediately "fails fast", routing
              traffic to a fallback cache. Periodically, it allows a single test request through
              (HALF-OPEN) to see if Service B has recovered.
            </p>
          </div>
          ...
          <div className="flex-1 space-y-2 w-full">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground font-mono bg-background p-2 rounded-md border border-hairline min-h-[60px] overflow-y-auto max-h-[80px]">
              ...
              {step === 4 &&
                "Step 5: Request 4 TIMEOUTS. Circuit Breaker failure threshold exceeded!"}
              {step === 5 &&
                "Step 6: Circuit Breaker snaps OPEN. Request 5 fails fast and routes to Cache."}
              {step === 6 && "Step 7: Request 6 fails fast. Service A resources are protected."}
              {step === 7 &&
                "Step 8: Timeout period expires. Breaker is HALF-OPEN. Sending test request to B."}
              {step === 8 &&
                "Step 9: Test succeeds! Service B has recovered. Breaker is CLOSED again."}
