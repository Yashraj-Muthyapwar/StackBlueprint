...
const osiLayers = [
  ...
  },
  {
    ...
    dataUnit: "Data",
    encapsulationExample: "Encrypts the HTTP request via SSL/TLS for secure transmission to Vercel.",
    decapsulationExample: "Decrypts the data (SSL/TLS). Ensures it's in a format the browser understands.",
  },
  {
    num: 5,
    name: "Session",
  ...
  },
  {
    ...
    dataUnit: "Packets",
    encapsulationExample: "Adds IP header with Source IP (You) and Dest IP (stackblueprint server).",
...
export function OsiModelDiagram() {
  ...
  const [hoveredState, setHoveredState] = useState<{ num: number, side: "sender" | "receiver" } | null>(null);
  const [selectedState, setSelectedState] = useState<{ num: number, side: "sender" | "receiver" } | null>(null);
  ...
  const activeSide = step === -1 ? null : step <= 6 ? "sender" : step === 7 ? "network" : step <= 14 ? "receiver" : "done";
  const activeLayerNum = step === -1 || step === 15 ? null : step <= 6 ? 7 - step : step === 7 ? 1 : step - 7;
  ...
  const displayState = selectedState !== null ? selectedState : hoveredState !== null ? hoveredState : activeLayerNum !== null ? { num: activeLayerNum, side: activeSide as "sender" | "receiver" } : null;
  const displayLayer = displayState !== null ? osiLayers.find(l => l.num === displayState.num) : null;
  ...
  return (
    <div className="my-10 w-full rounded-2xl border border-white/10 bg-[#0a0a0a] shadow-2xl relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 sm:p-6 border-b border-white/10 bg-white/[0.02]">
        <div>
          <h3 className="text-xl font-bold text-white/90">OSI Model: Encapsulation & Decapsulation</h3>
          <p className="text-sm text-white/50">Click a layer to lock details, or play the animation.</p>
        </div>
        <button 
        ...
        <div className="min-w-[860px] grid grid-cols-[280px_1fr_280px] gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            {osiLayers.map((layer) => {
              ...
              <div 
                ...
                onClick={() => setSelectedState(isLocked ? null : { num: layer.num, side: "sender" })}
              ...
              >
                <div className={`flex items-center justify-center w-8 h-8 rounded-lg font-mono font-bold text-sm shrink-0 transition-colors duration-300 ${activeSide === "sender" && activeLayerNum === layer.num ? `bg-${layer.color.replace('text-', '')}/20 ${layer.color}` : 'bg-white/5 text-white/40'}`}>
                  L{layer.num}
                </div>
                <div className={`shrink-0 transition-colors duration-300 ${(activeSide === "sender" && activeLayerNum === layer.num) || isLocked ? layer.color : 'text-white/40'}`}>
                  <layer.icon size={20} />
                </div>
                <div className={`flex-1 font-semibold truncate text-sm ${(activeSide === "sender" && activeLayerNum === layer.num) || isLocked ? 'text-white' : 'text-white/90'}`}>{layer.name}</div>
              </div>
            )})}
          </div>
          ...
          <div className="relative flex flex-col items-center">
             ...
             <div className="absolute top-0 w-full flex justify-between px-4 text-[10px] font-bold uppercase tracking-widest text-white/30">
               <span className="flex items-center gap-1"><ArrowDown size={12}/> Encapsulate</span>
               <span className="flex items-center gap-1">Decapsulate <ArrowUp size={12}/></span>
             </div>
                     ...
                     <div className="flex items-center gap-1.5 w-full justify-center py-0.5">
                       <AnimatePresence mode="popLayout">
                         {getActiveHeaders().map(h => (
                           <motion.div 
                             key={h.num} 
                             initial={{ opacity: 0, scale: 0.5, width: 0, marginLeft: 0, marginRight: 0 }}
                             animate={{ opacity: 1, scale: 1, width: "auto", marginLeft: 2, marginRight: 2 }}
                             exit={{ opacity: 0, scale: 0.5, width: 0, marginLeft: 0, marginRight: 0 }}
                       ...
                       {/* Core Data Block */}
                       <motion.div layout className="shrink-0 mx-0.5 px-3 py-1 rounded-[4px] bg-purple-600/30 border border-purple-600/50 text-purple-300 text-[10px] font-bold shadow-sm">
                         DATA (HTTP)
                       </motion.div>
                     </div>
                   )}
             ...
             <div className="absolute bottom-[24px] w-full h-[2px] bg-white/10 rounded-full flex items-center justify-center">
               ...
               {step >= 7 && step <= 8 && (
                  <motion.div 
                    ...
                    animate={step === 7 ? { width: "100%", left: "0%", opacity: 1 } : { width: "0%", left: "100%", opacity: 0 }}
          ...
          <div className="flex flex-col gap-2">
            ...
            {osiLayers.map((layer) => {
              const isLocked = selectedState?.num === layer.num && selectedState?.side === "receiver";
              return (
              <div 
                key={`receiver-${layer.num}`}
                onMouseEnter={() => setHoveredState({ num: layer.num, side: "receiver" })}
                onMouseLeave={() => setHoveredState(null)}
                onClick={() => setSelectedState(isLocked ? null : { num: layer.num, side: "receiver" })}
              ...
              >
                <div className={`flex-1 font-semibold truncate text-right text-sm ${(activeSide === "receiver" && activeLayerNum === layer.num) || isLocked ? 'text-white' : 'text-white/90'}`}>{layer.name}</div>
                <div className={`shrink-0 transition-colors duration-300 ${(activeSide === "receiver" && activeLayerNum === layer.num) || isLocked ? layer.color : 'text-white/40'}`}>
                  <layer.icon size={20} />
                </div>
                <div className={`flex items-center justify-center w-8 h-8 rounded-lg font-mono font-bold text-sm shrink-0 transition-colors duration-300 ${activeSide === "receiver" && activeLayerNum === layer.num ? `bg-${layer.color.replace('text-', '')}/20 ${layer.color}` : 'bg-white/5 text-white/40'}`}>
                  L{layer.num}
                </div>
              </div>
            )})}
      ...
      <AnimatePresence mode="wait">
        {displayLayer ? (
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex items-start justify-between gap-6 flex-col md:flex-row">
                <div className="flex-1">
                  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase mb-4 ${displayLayer.bgColor} ${displayLayer.color} border border-current/20`}>
                ...
                <div className="w-full md:w-64 space-y-3 shrink-0">
                  <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                    <h4 className="text-[9px] font-bold text-white/40 uppercase tracking-widest mb-1.5">Protocol Data Unit</h4>
                    <span className={`font-mono text-sm ${displayLayer.color}`}>{displayLayer.dataUnit}</span>
                  </div>
                  <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                    <h4 className="text-[9px] font-bold text-white/40 uppercase tracking-widest mb-1.5">Protocols</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {displayLayer.protocols.split(', ').map(p => (
                        <span key={p} className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-white/5 border border-white/10 text-white/80">
                          {p}
                        </span>
                      ))}
                    </div>
              ...
              <div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10 border-l-4" style={{ borderLeftColor: displayLayer.color.includes('purple') ? '#9333ea' : displayLayer.color.includes('blue') ? '#2563eb' : displayLayer.color.includes('teal') ? '#0d9488' : displayLayer.color.includes('green') ? '#16a34a' : displayLayer.color.includes('yellow') ? '#ca8a04' : displayLayer.color.includes('orange') ? '#f97316' : '#ef4444' }}>
                <h4 className="text-[9px] font-bold text-white/40 uppercase tracking-widest mb-1">
                  {displayState?.side === "receiver" ? "Decapsulation Example" : "Encapsulation Example"}
                </h4>
                <p className="text-sm text-white/80">
                  {displayState?.side === "receiver" ? displayLayer.decapsulationExample : displayLayer.encapsulationExample}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="w-full p-8 border-t border-white/5 bg-white/[0.01] flex flex-col gap-2 items-center justify-center min-h-[220px]">
            <p className="text-white/40 text-base font-medium">Hover over a layer to view details.</p>
