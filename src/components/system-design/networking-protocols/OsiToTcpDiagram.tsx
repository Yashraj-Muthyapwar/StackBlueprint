    ...
    <div className="flex flex-col rounded-xl border border-hairline bg-card/50 overflow-hidden shadow-sm my-6">
      ...
      <div className="relative p-6 md:p-10 bg-grid-slate-900/[0.04] dark:bg-grid-slate-50/[0.02] flex justify-center">
        <div className="relative flex w-full max-w-[700px] h-[384px]">
          ...
          <div className="relative w-[180px] md:w-[220px] h-full z-20">
            <h3 className="absolute -top-8 left-0 right-0 text-[10px] font-bold text-center text-muted-foreground uppercase tracking-wider">
              OSI Model (7 Layers)
            </h3>
          ...
          <div className="relative w-[180px] md:w-[220px] h-full z-20">
            ...
            {tcpLayers.map((layer, i) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm md:text-base font-bold ${layer.textColor}`}>
                    {layer.name}
                  </span>
                  <span
                    className={`text-[9px] md:text-[10px] font-mono mt-1 opacity-70 ${layer.textColor}`}
                  >
                    {layer.desc}
                  </span>
                </motion.div>
      ...
      <div className="border-t border-hairline bg-muted/20 p-4 md:p-6">
        <p className="text-xs md:text-sm leading-relaxed text-muted-foreground">
          The 7-layer OSI Model is a theoretical framework useful for understanding network flow and
          troubleshooting. However, real-world networks (like the internet) are built on the simpler
          4-layer <strong>TCP/IP Model</strong>. The top 3 OSI layers (Application, Presentation,
          Session) are handled by the application itself, combining them into one TCP/IP
          "Application" layer. The bottom 2 hardware layers (Data Link, Physical) are combined into
