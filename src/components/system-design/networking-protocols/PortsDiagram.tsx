...
export function PortsDiagram() {
  ...
  const getPacketPosition = () => {
    switch (packetState) {
      ...
      case "syn_out":
        return { x: -30, y: 0, opacity: 1, scale: 1 };
      case "syn_firewall":
        return { x: 70, y: 0, opacity: 1, scale: 1 };
      case "rst_in":
        return { x: -80, y: 40, opacity: 0, scale: 0.5 };
      case "syn_server":
        return { x: 120, y: 0, opacity: 1, scale: 1 };
      case "syn_ack_in":
        return { x: -30, y: 0, opacity: 1, scale: 1 };
      case "syn_ack_client":
        return { x: -180, y: 0, opacity: 1, scale: 1 };
      case "ack_out":
        return { x: 70, y: 0, opacity: 1, scale: 1 };
      case "ack_server":
        return { x: 120, y: 0, opacity: 1, scale: 1 };
        ...
        <div className="grid grid-cols-1 md:grid-cols-[220px_1fr_260px] gap-6 items-center">
          ...
          <div className="flex flex-col items-center gap-4 z-20">
            <div className="rounded-xl border border-hairline bg-surface-2/80 p-5 text-center shadow-lg backdrop-blur-md w-full relative overflow-hidden">
              ...
              <div className="mt-1 font-mono text-[10px] text-muted-foreground bg-black/10 dark:bg-white/5 py-1 px-2 rounded w-fit mx-auto border border-hairline/50 flex flex-col gap-0.5 relative group cursor-help">
                ...
                <span className="text-[9px] opacity-80 border-b border-dashed border-muted-foreground/50 pb-px">
                  Src Port (Ephemeral): {isSending ? sourcePort : "..."}
                </span>
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-48 p-2.5 bg-surface-2 border border-hairline rounded shadow-lg text-[10px] text-foreground opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 normal-case font-sans">
                  The OS automatically assigns a random high-numbered port (49152-65535) for each
                  outgoing request to uniquely identify the connection on the client side.
                </div>
              </div>

              <div className="mt-6 flex flex-col gap-2">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 text-left px-1">
                  Destination Port:
                </p>
                {SERVICES.map((s) => (
                  <button
                  ...
                  >
                    <s.icon
                      className={`size-4 ${selectedService.id === s.id ? s.color : "text-muted-foreground group-hover:text-foreground"}`}
                    />
                  ...
                  {packetState === "rst_in" ? (
                    ...
                    <>
                      <div className="bg-black/10 dark:bg-white/10 w-[110%] -mt-2 mb-2 py-0.5 text-[8px] font-bold text-center tracking-widest uppercase opacity-80">
                        ...
                        <span className="text-[7px] ml-0.5 px-0.5 border border-current rounded-sm opacity-90">
                          {getPacketFlag()}
                        </span>
                      </div>
                      <div className="flex flex-col w-full gap-0.5 border-b border-hairline/50 pb-1.5 mb-1.5">
                        <div className="flex justify-between items-center w-full">
                          ...
                          <span className="font-mono text-[8px]">
                            {["syn_ack_in", "syn_ack_client", "rst_in"].includes(packetState)
                              ? `203.0.113.45:${selectedService.port}`
                              : `192.168.1.5:${sourcePort}`}
                          </span>
                        </div>
                        <div className="flex justify-between items-center w-full">
                          <span className="font-mono text-[8px] text-muted-foreground">DST:</span>
                          <span
                            className={`font-mono text-[9px] font-bold ${selectedService.color}`}
                          >
                            {["syn_ack_in", "syn_ack_client", "rst_in"].includes(packetState)
                              ? `192.168.1.5:${sourcePort}`
                              : `203.0.113.45:${selectedService.port}`}
                      ...
                      <div
                        className={`grid size-6 place-items-center rounded-full ${selectedService.color} bg-white/5`}
                      >
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              ...
              >
                ...
                <motion.line
                  ...
                  stroke="currentColor"
                  className={
                    selectedService.isBlocked && packetState === "rst_in"
                      ? "text-red-500"
                      : "text-mint/50"
                  }
                  strokeWidth="2"
                  ...
                  animate={
                    isSending
                      ? {
                          strokeDashoffset: ["syn_ack_in", "syn_ack_client", "rst_in"].includes(
                            packetState,
                          )
                            ? 100
                            : -100,
          ...
          <div className="flex flex-col items-center gap-4 z-20">
            <div className="rounded-xl border border-hairline bg-surface/80 p-5 text-center shadow-lg backdrop-blur-md w-full relative overflow-hidden">
              ...
              <p className="mt-1 font-mono text-[10px] text-muted-foreground bg-black/10 dark:bg-white/5 py-1 px-2 rounded w-fit mx-auto border border-hairline/50">
                IP: 203.0.113.45
              </p>

              <div className="mt-6 flex flex-col gap-3 relative">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground text-left px-1">
                  Active Services
                </p>
                {SERVICES.filter((s) => !s.isBlocked).map((s) => {
                  const isTarget =
                    isSending && selectedService.id === s.id && packetState === "established";
                  return (
                    <motion.div
                      key={s.id}
                      animate={{
                    ...
                    >
                      <div
                        className={`grid size-8 shrink-0 place-items-center rounded-md bg-white/50 dark:bg-black/20 ${s.color} shadow-sm`}
                      >
            ...
            <p className="text-sm text-foreground/80 leading-relaxed font-mono">
              {isSending ? (
                <span className="animate-pulse">
                  {packetState === "syn_out" || packetState === "syn_firewall" ? (
                    `Initiating TCP Handshake. Sending SYN packet to port ${selectedService.port}...`
                  ) : packetState === "rst_in" ? (
                    <span className="text-red-500 font-bold">
                      Connection to port {selectedService.port} refused. Firewall returned TCP RST.
                    </span>
                  ) : packetState === "syn_server" ? (
                    `Firewall allowed SYN. Server evaluating request...`
                  ) : packetState === "syn_ack_in" || packetState === "syn_ack_client" ? (
                    `Server accepted connection. Returning SYN-ACK packet...`
                  ) : packetState === "ack_out" || packetState === "ack_server" ? (
                    `Client received SYN-ACK. Sending final ACK to establish connection...`
                  ) : packetState === "established" ? (
                    <span className="text-mint font-bold">{selectedService.successMessage}</span>
                  ) : (
                ...
                <span>
                  Select a destination port and send a request. Watch how the firewall processes or
                  rejects the incoming socket request.
                </span>
              )}
            </p>
          </div>
