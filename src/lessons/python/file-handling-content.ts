...
export const FILE_HANDLING_TOPICS: Record<
  string,
    ...
    lessons: [
      {
        ...
        title: "File Basics",
        subtitle:
          "Generate sample contacts with Faker and save them in a text file you can read tomorrow.",
        sections: [
          {
            kind: "prose",
          ...
          },
          {
            ...
            body: ["### 1. Generate three practice contacts", "Create `save_contacts.py` and add:"],
          },
          {
          ...
          {
            kind: "prose",
            body: ["### 2. Write the names to a new text file", "Add this code below the list:"],
          },
          {
            kind: "interactive-code",
          ...
          {
            kind: "prose",
            body: ["### 3. Read back what you saved", "Finish the script with:"],
          },
          {
            kind: "interactive-code",
          ...
          },
          {
            ...
            body: ["Here is the complete first version:"],
          },
          {
            kind: "interactive-code",
            ...
            questions: [
              {
                id: "fb-1",
                question:
                  "What remains after the script exits: the `contacts` list or `contacts.txt`?",
                options: ["The contacts list", "contacts.txt", "Both remain", "Neither remain"],
                correctIndex: 1,
                explanation:
                  "The contacts list in memory is cleared, but the file on disk remains.",
              },
              {
                id: "fb-2",
      ...
      },
      {
        ...
        subtitle:
          "Build, inspect, create, and find file locations without guessing which folder Python is using.",
        sections: [
          {
            kind: "prose",
            heading: "Why this matters",
          ...
          },
          {
            kind: "prose",
            heading: "Step-by-step",
            body: ["### 1. See where Python starts and build a path"],
          },
          {
            kind: "interactive-code",
          ...
          },
          {
            kind: "prose",
            heading: "A simple example",
            body: ["Create three small exports, then list their names:"],
          },
          {
            kind: "interactive-code",
            ...
            questions: [
              {
                ...
                correctIndex: 0,
                explanation:
                  "It doesn't start from the root directory, so it's a relative path starting from the current working directory.",
              },
              {
                id: "wp-2",
                ...
                correctIndex: 1,
                explanation:
                  "Creating a Path object only builds a representation of the path, it doesn't touch the file system.",
              },
              {
                id: "wp-3",
                question: "Which call reveals where relative paths begin?",
                options: ["Path.start()", "Path.root()", "Path.home()", "Path.cwd()"],
                correctIndex: 3,
                explanation:
                  "`Path.cwd()` returns the Current Working Directory, which is the starting point for relative paths.",
              },
              {
                id: "wp-4",
                question: "Which method searches nested folders?",
                options: ["glob()", "rglob()", "search()", "find()"],
                correctIndex: 1,
                explanation:
                  "`rglob()` recursively searches the current directory and all subdirectories.",
              },
            ],
          },
        ],
            ...
            questions: [
              {
                ...
                options: ["file.read()", "file.readlines()", "for line in file:"],
                correctIndex: 2,
                explanation:
                  "Looping directly over the file object only loads one line into memory at a time, making it safe for massive files.",
              },
              {
                id: "rf-2",
                ...
                correctIndex: 1,
                explanation:
                  "readline() returns the string exactly as it is in the file, including the trailing newline character.",
              },
            ],
          },
        ],
            ...
            questions: [
              {
                ...
                correctIndex: 0,
                explanation:
                  "Write mode ('w') is destructive and immediately truncates (wipes) the file.",
              },
              {
                id: "wf-2",
                question: "What will `file.write('A'); file.write('B')` output to the file?",
                options: ["A on line 1, B on line 2", "AB", "A B"],
                correctIndex: 1,
                explanation:
                  ".write() does not automatically add spaces or newlines, so they are joined together.",
              },
            ],
          },
        ],
            ...
            questions: [
              {
                ...
                correctIndex: 1,
                explanation:
                  "PDFs are binary files, not plain text, so you must use 'rb' (Read Binary).",
              },
              {
                id: "fm-2",
                ...
                correctIndex: 2,
                explanation:
                  "Exclusive creation ('x') is a safe alternative to 'w' that prevents accidental data loss.",
              },
            ],
          },
        ],
      },
      {
        ...
        sections: [
          ...
          },
          {
            kind: "prose",
            heading: "Step-by-step",
            body: ["### 1. Find your location with tell()"],
          },
          {
            kind: "interactive-code",
            code: `with open('poem.txt', 'r') as file:\n    print(file.tell()) # 0\n    file.read(5)\n    print(file.tell()) # 5`,
            ...
            questions: [
              {
                id: "fmet-1",
                question:
                  "If you run `file.read(10)`, what will `file.tell()` return immediately after?",
                options: ["0", "10", "An error"],
                correctIndex: 1,
                explanation:
                  "The read(10) method reads 10 bytes and advances the cursor exactly 10 bytes forward.",
              },
              {
                id: "fmet-2",
                ...
                correctIndex: 2,
                explanation:
                  "seek(0) jumps the cursor back to the beginning, allowing you to re-read the file without reopening it.",
              },
            ],
          },
        ],
            ...
            questions: [
              {
                ...
                options: [
                  ...
                  "DirEntry objects that can report type information and metadata",
                  "A list of files and folders",
                ],
                correctIndex: 1,
                explanation:
                  "os.scandir() is efficient because it yields DirEntry objects that often contain cached metadata like size and file type.",
              },
              {
                id: "os-2",
                ...
                correctIndex: 2,
                explanation:
                  "0o600 gives the owner read/write permissions and prevents all access for the group and others.",
              },
              {
                id: "os-3",
                ...
                options: [
                  ...
                  "When your code intentionally replaces an existing destination file",
                  "When working on Windows only",
                ],
                correctIndex: 1,
                explanation:
                  "os.replace() safely overwrites the destination file atomically (if they are on the same filesystem), whereas os.rename() can throw an error if the destination exists on some platforms.",
              },
            ],
          },
        ],
        ...
        sections: [
          ...
          },
          {
            ...
            rows: [
              ...
              [
                "number with a decimal point or exponent",
                "`float`",
                "Use `Decimal` when exact money values matter.",
              ],
              [
                "`true` / `false`",
                "`True` / `False`",
                "JSON uses lowercase; Python uses capitalized names.",
              ],
          ...
          {
            kind: "prose",
            body: ["The four core functions differ only by where text comes from or goes to:"],
          },
          {
            kind: "table",
            ...
            rows: [
              ...
              [
                "An open text file",
                "`json.load(file)`",
                "`json.dump(data, file)` writes to the file",
              ],
            ],
          },
          {
          ...
          },
          {
            ...
            questions: [
              ...
              },
              {
                ...
                correctIndex: 0,
                explanation:
                  "json.JSONDecodeError includes details about the exact line and column where parsing failed.",
              },
              {
                id: "json-v3-5",
                question: "Why validate after parsing?",
                options: [
                  "Parsing doesn't create Python objects",
                  "Valid JSON can still have missing keys or incorrect value types",
                  "Parsing only reads strings",
                ],
                correctIndex: 1,
                explanation:
                  "Syntax validation is not schema validation. You still need to ensure the structure meets your program's needs.",
              },
              {
                id: "json-v3-6",
                ...
                correctIndex: 0,
                explanation:
                  "indent=2 formats the JSON string with 2-space indentation and newlines.",
              },
              {
                id: "json-v3-7",
                ...
                correctIndex: 1,
                explanation:
                  "ensure_ascii=False prevents json.dumps() from escaping characters into \\uXXXX sequences.",
              },
              {
                id: "json-v3-8",
                question: "When is JSON Lines a better fit than a regular JSON array?",
                options: [
                  "For large or append-only independent records",
                  "When formatting for human review",
                  "When creating small configuration files",
                ],
                correctIndex: 0,
                explanation:
                  "JSON Lines keeps memory usage low by allowing you to process one record at a time, making it ideal for large datasets or logs.",
              },
            ],
          },
          {
          ...
          },
          {
            ...
            rows: [
              ...
              [
                ...
                "`csv.writer`",
                "Write lists or tuples with `writerow()` / `writerows()`.",
              ],
              [
                "Read header-based records",
                "`csv.DictReader`",
                "Each row is a dictionary keyed by the header.",
              ],
              [
                "Write declared columns",
                "`csv.DictWriter`",
                "Write dictionaries in a chosen column order.",
              ],
            ],
          },
          {
            ...
            questions: [
              {
                ...
                options: [
                  ...
                  "Fields can contain quoted commas, quotation marks, and newlines",
                  "It cannot parse UTF-8 characters",
                ],
                correctIndex: 1,
                explanation:
                  "Manual splitting fails when the comma is inside quotes, or when a field contains a newline.",
              },
              {
                id: "csv-v3-2",
                ...
                options: [
                  ...
                  "It lets the csv module handle CSV record endings correctly",
                  "It increases read performance",
                ],
                correctIndex: 1,
                explanation:
                  "The csv module needs direct control over newlines, otherwise Windows writes extra blank rows.",
              },
              {
                id: "csv-v3-3",
                ...
                options: [
                  "A list of strings",
                  "A dictionary keyed by header names",
                  "A Pandas DataFrame",
                ],
                correctIndex: 1,
                explanation:
                  "DictReader maps the first row (the header) to dictionary keys for all subsequent rows.",
              },
              {
                id: "csv-v3-4",
              ...
              },
              {
                ...
                correctIndex: 1,
                explanation:
                  "Building the map takes O(n), and scanning the second file takes O(m), making the average time O(n + m).",
              },
            ],
          },
          {
...

      {
        ...
        sections: [
          ...
          },
          {
            ...
            rows: [
              [
                "Convert a Python object to pickle data",
                "`pickle.dumps(object)` returns bytes",
                "`pickle.dump(object, binary_file)` writes bytes",
              ],
...
**Treating a restricted unpickler as a complete defense:** It is not a substitute for keeping untrusted pickle data out of the system.`,
            ...
            questions: [
              {
                ...
                options: ["A string", "A file object", "A bytes object", "A dictionary"],
                correctIndex: 2,
                explanation:
                  "The 's' in dumps stands for string, but in Python 3 it returns a bytes object containing the serialized data.",
              },
              {
                id: "pickle-2",
                question: "Which file modes are appropriate for pickle \`dump\` and \`load\`?",
                options: ["'w' and 'r'", "'wb' and 'rb'", "'a' and 'r+'", "'x' and 'w'"],
                correctIndex: 1,
                explanation:
                  "Pickle files are binary data, so you must use 'wb' to write and 'rb' to read.",
              },
              {
                id: "pickle-3",
                ...
                correctIndex: 2,
                explanation:
                  "Unpickling allows execution of code (like calling functions). Malicious data can exploit this.",
              },
              {
                id: "pickle-4",
                ...
                correctIndex: 1,
                explanation:
                  "A cache version ensures that if your program's data requirements change, it will safely ignore the old, incompatible cache.",
              },
              {
                id: "pickle-5",
                ...
                correctIndex: 2,
                explanation:
                  "Atomic replacement ensures that any reader opening the cache file will either see the complete old file or the complete new file, but never a corrupted half-written file.",
              },
            ],
          },
        ],
      },
      {
        ...
        sections: [
          ...
          {
            kind: "table",
            headers: ["You need to...", "Prefer", "Why"],
            rows: [
              [
              ...
              ["Copy a file or folder tree", "`shutil`", "Performs the complete copy operation."],
              [
              ...
              ["Delete one known file", "`Path.unlink()`", "A smaller, targeted operation."],
            ],
          } as const,
          {
            kind: "prose",
          ...
          {
            kind: "table",
            headers: ["Function", "Destination can be a folder?", "Metadata", "Good use"],
            rows: [
              ["`shutil.copyfile()`", "No", "File bytes only", "An exact output filename."],
              ["`shutil.copy()`", "Yes", "File mode, not all metadata", "A normal working copy."],
              [
                "`shutil.copy2()`",
                "Yes",
          ...
          } as const,
          {
            ...
            rows: [
              ...
              ["Disk space", "Check free space before a large copy or archive."],
              [
                "Archive trust",
                "Only unpack archives from trusted sources. Establish a security policy for downloaded archives.",
              ],
              ["Deletion", "Keep `rmtree()` targets narrow, known, and logged. There is no undo."],
            ],
          } as const,
          {
            kind: "callout",
            ...
            questions: [
              {
                id: "shutil-1",
                question:
                  "Which function is a good default for a backup that should preserve timestamps where possible?",
                options: ["shutil.copyfile()", "shutil.copy()", "shutil.copy2()", "shutil.move()"],
                correctIndex: 2,
                explanation:
                  "shutil.copy2() preserves as much metadata (including timestamps) as the platform supports.",
              },
              {
                id: "shutil-2",
                ...
                correctIndex: 3,
                explanation:
                  "A move effectively relocates the file; whether via rename or a copy-then-delete, the source file is removed from its original path.",
              },
              {
                id: "shutil-3",
                question:
                  "Why is `rmtree()` treated differently from normal cleanup operations like `Path.unlink()`?",
                options: [
                ...
                correctIndex: 1,
                explanation:
                  "shutil.rmtree() permanently deletes a directory and its entire contents without any recycle bin step, making it highly destructive if misused.",
              },
            ],
          } as const,
        ],
