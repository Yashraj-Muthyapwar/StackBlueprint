...
function build({ arr: input }: Inputs): Step[] {
  ...
  push({
    ...
    mid: 0,
    high: n - 1,
    narration: "Sort 0s, 1s, 2s in one pass.",
  });
  let low = 0,
    mid = 0,
    high = n - 1;
  push({
    ...
    mid,
    high,
    narration: "Three pointers: low (0-zone), mid (scanner), high (2-zone).",
  });
  ...
  while (mid <= high && safety++ < 500) {
    ...
    if (arr[mid] === 0) {
      ...
      push({
        ...
        mid,
        high,
        narration: "It's a 1 — already correct, advance mid.",
      });
      mid += 1;
    } else {
      [arr[mid], arr[high]] = [arr[high], arr[mid]];
  ...
  push({
    ...
    mid,
    high,
    narration: "Partitioned into three zones.",
  });
  return steps;
}
